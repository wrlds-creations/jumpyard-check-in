...
const PROFILE_ICONS: JumpyardIconName[] = [
    'connected-band',
    'trampoline-jump',
    'child',
    'group',
...
export const ConnectedProfiles = ({ count, onContinue }: ConnectedProfilesProps) => {
    ...
    const [profiles, setProfiles] = useState<ConnectedProfile[]>(
        Array.from({ length: slots }, (_, i) => ({ id: i + 1, name: '', icon: PROFILE_ICONS[i % PROFILE_ICONS.length] }))
    );
    ...
    return (
        ...
        >
            <div className="flex items-center gap-2 mb-0.5">
                <JumpyardIcon name="connected-band" className="w-8 h-8" />
                            ...
                            {PROFILE_ICONS.map(ic => (
                                <button
                                    ...
                                    className={`w-8 h-8 rounded-full border flex items-center justify-center transition-all ${
                                        p.icon === ic ? 'bg-white border-primary scale-110 shadow-sm' : 'bg-white border-border opacity-55 hover:opacity-100'
                                    }`}
                                >
                                    <JumpyardIcon name={ic} className="w-6 h-6" />
                                </button>
                            ))}
                        </div>
                    </div>
