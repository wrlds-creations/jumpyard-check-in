...
Phone new-purchase drafts may carry the optional Cloud-only `emailMarketingConsent` object: `{ granted: true, copyVersion: "phone-email-2026-09-23-v3", locale: "sv" | "en" }`. It is never part of a quote. Under D0225 a valid choice sets `acceptMarketing: true` on the ROLLER customer of that phone draft; absence sends no email instruction, and SMS acceptance is never sent. A separately approved September 23 unpaid draft preserved all returned fields for the owned already-subscribed ROLLER guest; this observation does not establish every guest state, concurrent updates or downstream suppression behavior. Kiosk and existing-booking add-on contracts are unchanged.
