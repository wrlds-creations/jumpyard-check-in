...
## Pre-arrival email and session lifetime (#392)

New regular and provisional kiosk check-in sessions last four hours from creation; resuming preserves the existing expiration and safety/handoff state. Link validity remains 72 hours by default, email-link guest access remains one hour per successful link opening, and provisional kiosk guest credentials retain their existing two-hour recovery behavior. Staff authentication is unchanged. Completed product collection remains #345 scope.

The authenticated `POST /v1/check-in/session-links/send-due-messages` accepts `messagePolicy: "prearrival_email_v1"` for read-only T-120 planning only. It resolves the booking owner rather than an arbitrary ticket participant, returns aggregate page counts and an authenticated continuation cursor, and never exposes an email or link token. Only the park-test scheduler may deliver: each five-minute run walks every page with one clock inside a 40-second budget, and rows it cannot reach stay unreserved for the next run. Delivery is limited to Nacka visits on 2026-09-28 with one Entré/Weekday Combo line and otherwise only add-on or café lines, never for our own phone/kiosk purchases or bookings with a started check-in session. A controlled proof on 2026-09-25 reaches only bookings whose contact is love@ or love+tag@wrlds.com. [Full contract](docs/gh-392-prearrival-email.md).
...
Phone and kiosk (D0226, #444) new-purchase drafts may carry the optional Cloud-only `emailMarketingConsent` object: `{ granted: true, copyVersion: "phone-email-2026-09-23-v3", locale: "sv" | "en" }`. It is never part of a quote. Under D0225 a valid choice sets `acceptMarketing: true` on the ROLLER customer of that draft; absence sends no email instruction, SMS acceptance is never sent, and the kiosk cost-verification payload carries neither flag. A separately approved September 23 unpaid draft preserved all returned fields for the owned already-subscribed ROLLER guest; this observation does not establish every guest state, concurrent updates or downstream suppression behavior. Existing-booking add-on contracts are unchanged. Kiosk evidence uses source `kiosk_new_booking`.
...
- The existing-booking kiosk card-present path uses the same installation proof, fixed profile and persistent terminal reservation as new bookings. Cloud also checks the original booking venue, re-quotes without terminal data, and verifies exact amount plus SEK evidence before returning a terminal-bound JWT. Its temporary `primary` compatibility follows the same rollout boundary.
