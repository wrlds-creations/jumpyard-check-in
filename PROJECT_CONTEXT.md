...
T0100 is the deploy and integrated-smoke gate for the T0099 `Klippkort` implementation. It should not add new business behavior; it should deploy the booking Lambda change, verify the public phone app bundle exposes `Klippkort`, and prove invalid-code, entry-only full coverage, mixed entry-plus-add-ons, no-code, and gift-card regressions in Roller Playground/public dev. On 2026-06-04, T0100 deployed the booking Lambda to AWS dev with a diff limited to `BookingHandler` code and passed backend smokes: invalid codes stay rejected, the masked paid `10-Kort` ticket/code from booking `5101046` reduces eligible entry to `0 kr`, mixed entry plus JumpSocks leaves the add-on payable, and no-payment publish created Playground booking `5101133`. Public phone smoke remains pending because `https://jumpyard-check-in.pages.dev` still serves a bundle without `Klippkort` until the changes are committed, merged, and published.
