...
| SMS/email readiness | In focus | EventBridge due-SMS schedule exists but dev config keeps `confirmSend=false`; SNS account is still sandboxed. T0065 confirmed manual SMS delivery to the verified sandbox phone using the public Cloudflare check-in URL. T0066 confirmed email dry-run/audit behavior and SES readiness status, but SES remains sandboxed with no verified sender/domain identity. | Complete T0067 unified booking-time messaging before environment/cutover tickets; SMS production still needs sandbox/recipient policy, consent/unsubscribe wording, and production sender decisions. Email production still needs a verified SES sender/domain, recipient/sandbox policy, and final sender/reply-to/consent text. |
...
- Do not enable real unattended SMS or real email sends in T0066.
- Do not create or verify SES sender/domain identities in T0066 without explicit sender/domain approval.
- Do not change payment, staff auth, redeem, webhook, Data API, or SMS scheduling behavior in T0066.
