...
On 2026-06-01 Pabel confirmed that Roller fixed the Playground payment integration challenge and that card payments should now show up. T0075 confirmed the fix: the public checkout at `https://jumpyard-check-in.pages.dev` rendered `Kortbetalning`, the Adyen Visa test card ending `1142` submitted through the vendored Roller payment package, and the phone flow reached the safety-video step. The current payment-method list is controlled by Roller/Adyen configuration, not JumpYard filtering; after the card fix the rendered methods are card, Delbetalning, and Google Pay, while Swish is no longer visible and Apple Pay still needs Roller/Pabel confirmation. The near-term roadmap is therefore re-ordered to prove core guest flows before continuing SMS/email production unlock. T0076 proves the full new-booking purchase flow, T0077 proves the paid existing-booking check-in path, T0078 proves existing-booking add-product payment, T0079 removes duplicate add-product contact entry and adds payment-approved continuation polish, T0080 re-checks Data API/webhook/Aurora sync, and T0081 runs a full integrated flow rehearsal. T0081 found that new booking, webhook, Aurora, staff handoff, and staff redeem work together, but add-product draft creation without visible contact re-entry still fails closed when JumpYard Cloud cannot resolve full customer details for the original booking. T0082 should fix that add-product contact-resolution gap before SMS/email production unlock resumes.
...
| Payment production readiness | In focus | Swish worked publicly in T0054; Pabel confirmed on 2026-06-01 that the Playground payment integration issue is fixed; T0075 confirmed card renders; T0076 and T0081 confirmed full card-paid new-booking flow reaches ready-for-staff; T0081 confirmed staff redeem from that flow; T0078 confirmed add-product payment before T0079 contact-removal; T0081 found post-T0079 add-product draft creation now needs stronger server-side original-customer resolution before it can pay without visible contact re-entry. Existing JumpYard code uses Roller's approved payment package with response-only `paymentJwt`. | Keep the Roller package path. Fix add-product customer resolution in T0082, then re-run the linked add-product payment rehearsal. Swish/Apple Pay visibility remains a Roller/Pabel follow-up. |

## Non-Goals For Current Ticket

- Do not build new app behavior or change UI files unless a T0076 smoke blocker is found.
