...
T0103 should fix add-on availability gating before the Playground demo. The current availability response is enough for entry/family product selection but not enough for capacity-bound add-ons such as SkyRider. T0103 should extend the frontend/backend availability path so the add-ons step can disable or hide add-ons that are not available for the selected time, while keeping quote/draft availability validation as the server-side safety net.
