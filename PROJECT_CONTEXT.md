...
The post-T0089 roadmap moves gift cards and multi-visit passes ahead of broader production readiness. The business goal is not to sell gift cards in this pilot; it is to let a guest pay for a buy-entry booking with a gift card and to confirm whether multi-visit passes can be used as a payment/entitlement source in the same checkout journey. T0090 should discover the exact Roller Playground behavior and API payloads first. T0091 should implement only the proven paths. T0092 should run integrated smokes that prove normal card payment, full/partial gift-card payment, and multi-visit behavior through phone flow, Aurora, Roller Playground, check-in session, and staff redeem/eligibility.
...
| How should gift cards and multi-visit passes work in buy-entry checkout? | Gift cards should reduce or cover the amount due during booking costs/draft/payment. Multi-visit passes may be a payment method, entitlement, ticket, membership, or guest pass concept, and must not be guessed into the redeem model. | `T0090` | `Open` |
