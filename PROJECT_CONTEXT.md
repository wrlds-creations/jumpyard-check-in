...
T0093 validated the code-based Nacka `10-Kort` hypothesis with only safe Roller Playground `POST /bookings/draft/costs` calls. The supported payload shape is `discounts: [{ code }]`. Invalid codes, the paid `10-Kort` booking reference, booking unique id, booking item id, and a normal paid entry ticket id all returned no amount reduction. The paid `10-Kort` ticket id from booking `5101046` did reduce a `200 kr` entry quote to `amountOwing=0` and also reduced a two-person `400 kr` quote to `amountOwing=0`; Roller returned this as a normal `percentOff=100` discount with empty `multiPassAllocations`. V1 can support membership/`10-Kort` as code validation only, not remaining-balance display. A code should count as accepted only when Roller reduces `amountOwing` or returns a positive discount amount. Before any write smoke or UI implementation consumes a real pass/code, ask for explicit user approval and confirm with Roller whether draft/publish/payment enforces pass exhaustion and usage.
...
| How should gift cards and multi-visit passes work in buy-entry checkout? | Gift cards should reduce or cover the amount due during booking costs/draft/payment. Multi-visit likely needs Roller validation through a membership/discount-code-like path for Nacka and must not be guessed into the redeem model or displayed as remaining balance. | `T0091/T0093/Josh/Joao/Pabel` | `Partially answered: gift-card payload accepted and active full/partial fixtures work; paid 10-Kort did not expose/apply as beta multi-pass; T0093 confirmed 10-Kort ticket id works as discount-code-style costs validation but no balance/allocation is exposed` |
