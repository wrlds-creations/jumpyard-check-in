...
| Payment production readiness | In focus | Swish worked publicly in T0054; Pabel confirmed on 2026-06-01 that the Playground payment integration issue is fixed; T0075 confirmed card renders; T0076 confirmed the full card-paid new-booking flow reaches ready-for-staff; T0077 confirmed a paid existing booking resumes to QR/handoff without another payment. Existing JumpYard code uses Roller's approved payment package with response-only `paymentJwt`. | T0078 must now prove existing-booking add-product payment around this payment path. Do not build a custom card form or bypass the Roller package. |
