...
Before guest messaging production unlock resumes, the roadmap now includes polish/deployment plus real-time name enrichment tickets. T0087 should make the staff/admin app available through a Cloudflare URL and verify staff login, queue, QR/search, detail, and redeem outside local `localhost`. T0088 should investigate and implement the best same-day guest-name enrichment path for non-JumpYard-created bookings when webhook/live lookup sees a booking before the daily Data API customer import has run. Guest messaging production unlock moves to T0089.
