...
Before guest messaging production unlock resumes, the roadmap now includes two additional polish/deployment tickets. T0086 should clean up remaining UI inconsistencies, including incorrect fonts and the unnecessary backup-code box on the guest ready-for-staff screen. T0087 should make the staff/admin app available through a Cloudflare URL and verify staff login, queue, QR/search, detail, and redeem outside local `localhost`.
...
- Do not build new app behavior or change UI files in T0082.
...
- Do not change staff auth, redeem, webhook, Data API importer code, CDK infrastructure, or Aurora migrations.
