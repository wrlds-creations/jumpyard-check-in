...
T0093 validated the first code-based Nacka `10-Kort` hypothesis without writes. T0097 rechecked that hypothesis after Gustav clarified that the current Nacka `10-Kort` setup is not Roller beta multi-pass. The confirmed V1 model is membership/discount-code validation: send the guest-entered code to Roller as `discounts: [{ code }]` in Booking Costs/Create Draft Booking, treat it as accepted only if Roller reduces `amountOwing` or returns a positive discount amount, and do not show remaining visits. The paid `10-Kort` fixture booking `5101046` still returns zero balances from `GET /customers/4045520/multi-passes`, while the masked `10-Kort` code reduces a `200 kr` entry quote to `amountOwing=0`. The next step must be a controlled write smoke before UI implementation to prove whether draft/publish consumes a use and how Roller exposes exhaustion/readback.
...
T0093 validated the code-based Nacka `10-Kort` hypothesis with only safe Roller Playground `POST /bookings/draft/costs` calls. T0097 confirmed the same model after Gustav's product clarification and official Roller docs research. The supported payload shape is `discounts: [{ code }]`. Invalid codes, the paid `10-Kort` booking reference, booking unique id, booking item id, and a normal paid entry ticket id all returned no amount reduction. The paid `10-Kort` ticket/code from booking `5101046` reduced a `200 kr` entry quote to `amountOwing=0` and also reduced a two-person `400 kr` quote to `amountOwing=0`; Roller returned this as a normal `percentOff=100` discount with empty `multiPassAllocations`. V1 can support membership/`10-Kort` as code validation only, not remaining-balance display. A code should count as accepted only when Roller reduces `amountOwing` or returns a positive discount amount. Before any write smoke or UI implementation consumes a real pass/code, ask for explicit user approval and confirm whether draft/publish/payment enforces pass exhaustion and usage.
