...
Before guest messaging production unlock resumes, the roadmap now includes staff-admin deployment plus real-time name enrichment tickets. T0088 should investigate and implement the best same-day guest-name enrichment path for non-JumpYard-created bookings when webhook/live lookup sees a booking before the daily Data API customer import has run. Guest messaging production unlock moves to T0089.
