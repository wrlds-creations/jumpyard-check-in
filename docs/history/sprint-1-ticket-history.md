# Sprint 1 Ticket History

This archive was created in T0128 to move ticket-by-ticket implementation narrative out of active project context while preserving the old source-of-truth text.

Source file: `PROJECT_CONTEXT.md`

## Archived PROJECT_CONTEXT.md

The content below is the previous active `PROJECT_CONTEXT.md` before T0128 compressed it to stable current project memory.
...
T0093 validated the first code-based Nacka `10-Kort` hypothesis without writes. T0097 rechecked that hypothesis after Gustav clarified that the current Nacka `10-Kort` setup is not Roller beta multi-pass. T0098 then ran one controlled write smoke: the masked paid `10-Kort` code created and published Playground booking `5101114` with `amountOwing=0`, but Roller still exposed no remaining-use balance through `GET /customers/{customerId}/multi-passes`, Booking Costs, booking readback, or `/data/bookingitems`. Safe product-coverage quotes matched Gustav's explanation: the code discounts eligible entry/session pass products such as `Entré 60 min` and `Entré 120 min`, including multi-quantity/two-person entry quotes, but it does not discount add-ons such as JumpSocks, coffee/tea, or SkyRider. The confirmed V1 model is membership/discount-code validation only: send the guest-entered code to Roller as `discounts: [{ code }]`, treat it as accepted only if Roller reduces `amountOwing` or returns a positive discount amount, and do not show "10 kvar", "9 kvar", or any locally calculated remaining visits.

T0099 implements that model in the buy-entry phone checkout as an optional `Klippkort` field, not a `10-Kort`-specific field. The phone app sends the entered code to JumpYard Cloud, JumpYard Cloud forwards it to Roller as a `discounts: [{ code }]` payload, and the API returns only safe masked/applied metadata. A code is treated as accepted only when Roller returns a positive discount/amount reduction; no-effect codes block the review step. Full coverage by gift card or klippkort uses the same no-payment draft publish path. The UI still must not show remaining visits.

T0100 was the deploy and integrated-smoke gate for the T0099 `Klippkort` implementation. It deployed the booking Lambda change, verified the public phone app bundle exposes `Klippkort`, and proved invalid-code, entry-only full coverage, mixed entry-plus-add-ons, no-code, and gift-card regressions in Roller Playground/public dev. On 2026-06-04, T0100 deployed the booking Lambda to AWS dev with a diff limited to `BookingHandler` code and passed backend smokes: invalid codes stay rejected, the masked paid `10-Kort` ticket/code from booking `5101046` reduces eligible entry to `0 kr`, mixed entry plus JumpSocks leaves the add-on payable, and no-payment publish created Playground booking `5101133`. PR #99 merged the changes to `main`; after Cloudflare published, `https://jumpyard-check-in.pages.dev` assets contained `Klippkort`, `clipCard`, and `discountCodes`, and public API smoke reconfirmed invalid, entry-only, and mixed entry-plus-add-on outcomes.

T0101 adds the first dev operations runbook layer without changing AWS resources. Read-only AWS checks confirmed dashboard `jumpyard-check-in-dev-ops` exists, the 17 current `jumpyard-check-in-dev-*` CloudWatch alarms are `OK`, and the six main Lambda log groups have 30-day retention. `OPERATIONS_RUNBOOK.md` now maps signals, AWS console locations, Aurora checks, safe first actions, and escalation routing for Data API sync, webhook processing, booking quote/draft/payment, gift card/Klippkort, SMS/email, and staff handoff/redeem. Production/staging work still needs notification routing, channel-specific delivery alarms, Aurora/scheduler health thresholds, route protection, retention, and cutover runbooks.
...
- Pabel confirmed on 2026-06-01 that the Playground payment integration issue is fixed and card payments should now show up; T0075 confirmed the public card smoke, and T0076 confirmed the full card-paid new-booking flow.
...
- payment card/scheme was parked on Roller/Adyen configuration until Pabel confirmed on 2026-06-01 that the Playground issue is fixed; T0075 and T0076 now confirm the card path through the public new-booking flow
...
On 2026-06-01 Pabel confirmed that Roller fixed the Playground payment integration challenge and that card payments should now show up. T0075 confirmed the fix: the public checkout at `https://jumpyard-check-in.pages.dev` rendered `Kortbetalning`, the Adyen Visa test card ending `1142` submitted through the vendored Roller payment package, and the phone flow reached the safety-video step. The current payment-method list is controlled by Roller/Adyen configuration, not JumpYard filtering; after the card fix the rendered methods are card, Delbetalning, and Google Pay, while Swish is no longer visible and Apple Pay still needs Roller/Pabel confirmation. The near-term roadmap is therefore re-ordered to prove core guest flows before continuing SMS/email production unlock. T0076 proves the full new-booking purchase flow, T0077 proves the paid existing-booking check-in path, T0078 proves existing-booking add-product payment, T0079 removes duplicate add-product contact entry and adds payment-approved continuation polish, T0080 re-checks Data API/webhook/Aurora sync, T0081 runs a full integrated flow rehearsal, and T0082 fixes the remaining no-duplicate-contact add-product draft blocker for JumpYard-created bookings.

T0076 confirmed the public new-booking guest path end-to-end on `https://jumpyard-check-in.pages.dev`: buy entry, choose time, choose 60 min entry, skip add-ons, fill contact, review the basket before payment, pay by card with the Adyen Visa test card ending `1142`, continue through the safety video and all safety confirmations, and reach the ready-for-staff QR/handoff screen. The successful smoke used booking-time `11:00` and produced handoff code `JY4704`. The phone app called JumpYard Cloud for availability, quote, draft creation, lookup, session creation, and ready-for-staff state; the frontend still did not call Roller REST directly or handle Roller credentials. A short `404` lookup retry immediately after payment was observed before the paid Roller booking became visible through the normal lookup path, then the flow continued successfully. Direct Aurora readback could not be performed in this ticket because the local AWS SSO token for profile `wrlds-dev` had expired.

T0077 confirmed the public paid existing-booking happy path using booking reference `5100930` for the same operating date. Data API read-only lookup first confirmed that the booking was `Paid`, dated `2026-06-01`, and started at `11:00`. The phone app existing-booking path then found the booking through JumpYard Cloud and resumed directly to the ready-for-staff QR/handoff screen with code `JY4704`, because T0076 had already completed the safety flow for that session. This is the expected resume behavior: a guest who has already completed safety should not be asked to pay again or repeat completed safety steps. No staff redeem was performed in T0077.

T0078 confirmed the public existing-booking add-product payment path using paid booking reference `5100926` for the same operating date. The phone app found the booking through JumpYard Cloud, entered the add-ons step, selected one `Strumpor` item, collected the Roller-required add-on contact fields, showed a server-side quote of `45 kr`, created a separate linked add-on draft in `separate_draft_booking` mode, paid by card with the Adyen Visa test card ending `1142`, and returned to the original booking's safety-video continuation. The safe captured identifiers were original booking `5100926`, Roller draft unique id `fe892301-95b7-490a-b4ad-dff311cfdd7f`, prepayment draft `jypd_529c13ed3a8a4d83a1`, add-on group `jyao_32bbe440269649e7af`, and link `jyl_77074c7ce26047b3b0`; raw payment JWTs, access tokens, full contact values, and full card data were not persisted or printed. During selector/debug retries, unpaid pending linked add-on drafts were also created for bookings `5100929`, `5100928`, and `5100927`; the final successful pass was `5100926`. Direct Aurora readback could not be performed because the local AWS SSO token for profile `wrlds-dev` had expired.
...
T0081 rehearsed the current public Playground system end to end after T0080. A new card-paid public booking `5100963` for `2026-06-01 14:00` reached ready-for-staff with check-in session `jycs_mpv4s30n_b9f8b58c`, handoff code `JY7597`, and ticket `5100963-21683812`; staff auth/list/detail saw the same session, and staff-confirmed redeem succeeded with Roller status code `200`, leaving local session status `redeemed` and handoff status `completed`. A second new card-paid booking `5100965` was created as an add-product rehearsal source and reached Aurora/webhook state `Paid`, `fresh`, `roller_webhook_enrichment`, with ticket `5100965-21683813`. Existing-booking add-product UX correctly skipped duplicate visible contact entry and the quote path returned `45 kr`, but confirmed add-product draft creation without a customer payload failed closed with `customer.firstName is required for Roller draft booking creation` for both older booking `5100926` and fresh booking `5100965`. The next ticket must make add-product draft creation resolve or carry full original booking customer details server-side while keeping the guest-facing duplicate contact form removed.

T0082 fixes the T0081 add-product contact-resolution gap for bookings created through JumpYard Cloud. The booking Lambda now carries Roller `customerId` into local contact lookup, reuses the original new-booking draft contact stored in `jumpyard.prepayment_booking_drafts` when the original booking was created by JumpYard Cloud, and still falls back to Aurora `guest_profiles` when a Roller customer id is available. The guest-facing add-product flow still sends no customer payload and no duplicate contact form is restored. If email/phone cannot be resolved from server-side state, the add-product draft path still fails closed instead of inventing contact data. A deployed dev smoke for original booking `5100965` created a no-customer linked add-on draft and Aurora link row without printing raw `paymentJwt` or full contact values.

T0083 improves the authenticated staff handoff identity/search slice. The session Lambda list/detail routes now return staff-only guest identity fields with booking/customer name when available plus masked email/phone, and the list route can search server-side by handoff code, booking reference, stored name, email, or phone. The admin UI shows the identity data in queue/detail without adding raw contact values to the browser response, and ticket rows now show product names before raw Roller ticket ids. Roller Data API `/data/customers` was confirmed to include `firstName` and `lastName`; T0083 stores those values in `guest_profiles.latest_booking_context`, adds `customer_first_name`/`customer_last_name` to safe prepayment draft metadata, and backfills existing drafts from matched guest profiles where possible. A controlled smoke created a ready-for-staff session for paid booking `5100965`; search by booking reference, first name, derived last-name value, and masked contact matched it, while the response confirmed raw `email` and raw `phone` fields are not returned. Remaining real-time name gaps for non-JumpYard-created bookings are tracked as a follow-up.

T0084 improves the staff/admin handoff page as a frontend-only operational UX step. The same staff-authenticated APIs remain in use, but the page now prioritizes one work surface: search or QR scan plus queue when no handoff is selected, then a focused compact summary view when staff selects/searches/scans a handoff. The focused summary has an X/back control, hides masked contact details, avoids repeating booking/safety tiles, shows only date/time/payment as top facts, and keeps products in a compact "att lämna ut" list. The separate visible ticket list and large post-redeem success confirmation remain deferred until they are operationally needed, with the success confirmation still planned for T0085. As a user-approved pulled-forward T0086 fix, the guest ready-for-staff screen no longer shows a separate backup-code box; it still shows the QR code and main staff/pickup code.

T0085 improves the staff/admin completion moment without changing backend, staff auth, or Roller redeem semantics. After the existing staff redeem call succeeds, the admin UI now shows a large green check-in confirmation with guest name, handoff code, booking reference, ticket count, and status. The confirmation stays visible until staff chooses `Tillbaka till kön` or `Scanna ny QR`; both paths clear the selected handoff/search state, remove the redeemed session from the visible queue, and keep staff on the operating surface for the next guest.

T0086 runs a narrow guest/admin UI cleanup before broader testing. The active phone/admin source no longer contains backup-code labels or backup-code copy; the legacy phone present-code component now labels the short code as staff/personalkod if it is ever reached; and unused `font-stretch-expanded` helpers were removed from phone/admin globals. The documented system sans-serif stack remains the only active app font rule, and no backend, AWS, Roller, payment, SMS, email, package, asset, or flow behavior changed.

T0087 prepares the staff/admin app for a dedicated Cloudflare Pages URL. The confirmed dev admin origin is `https://jumpyard-checkin-admin.pages.dev`, the admin app now has Cloudflare Pages setup docs and static security headers, and the dev API CORS config includes that origin. The dev API CORS change was deployed to AWS on 2026-06-02, and preflight for `POST /v1/staff/auth/login` now returns `access-control-allow-origin: https://jumpyard-checkin-admin.pages.dev`. Public staff smoke passed from the Cloudflare URL: staff login worked, the ready queue loaded, the handoff detail opened for booking `5100992`, staff redeem completed booking `5100992`/handoff `JY9056`, the queue returned to `0`, and QR scanner mode opened without a backend write.

T0088 adds real-time guest-name enrichment for webhook-updated bookings. The webhook still starts with server-side Roller booking detail, and now uses the documented read-only `GET /guests/{guestId}` fallback only when booking detail has a customer/guest id but not enough structured guest identity. The webhook upserts `jumpyard.guest_profiles` with Roller customer id, first/last name in `latest_booking_context`, masked/hashed email and phone, and connects booking summaries/tickets through `bookingCustomerId`/`roller_customer_id`. Deployed dev smoke for booking `5100965` returned `guestDetailStatus=available`, `guestNamePresent=true`, and Aurora booleans confirmed customer id, booking-name flag, guest profile, first/last context, email, and phone were present without printing raw PII.

T0089 prepares the guest SMS/email production unlock package without enabling production sends. Read-only AWS checks on 2026-06-02 confirmed SNS SMS `IsInSandbox=true`, SNS SMS `DefaultSMSType=Transactional`, `MonthlySpendLimit=1`, no SMS Sender ID, no SMS pools/origination numbers, AWS End User Messaging `ACCOUNT_TIER=SANDBOX`, SES `ProductionAccessEnabled=false`, SES sandbox quota `200/day` and `1/sec`, and only `love@wrlds.com` verified as an SES email identity. `GUEST_MESSAGING_PRODUCTION_UNLOCK.md` now documents SMS sandbox exit, SES production access, sender/domain identity requirements, missing user inputs, and hard gates. EventBridge due-message processing remains planning-only with `confirmSend=false`.

Before guest messaging production unlock can become an implementation/deploy ticket, JumpYard/WRLDS must provide final public URL/custom domain, SMS/email copy, volume/peak-rate assumptions, destination countries, consent/support wording, sender display goal, email from domain/address, DNS access, and explicit approval to submit AWS support cases or configure sender identities.

The post-T0089 roadmap moves gift cards and multi-visit/code handling ahead of broader production readiness. The business goal is not to sell gift cards in this pilot; it is to let a guest pay for a buy-entry booking with a gift card and to confirm whether membership/multi-visit codes can be validated by Roller in the same checkout journey. T0090 should discover the exact Roller Playground behavior and API payloads first. T0091 should implement only the proven gift-card path. T0092 should run integrated gift-card smokes through phone flow, Aurora, Roller Playground, check-in session, and staff redeem/eligibility. T0093 should validate membership/multi-visit code handling separately without promising or displaying remaining visit balance.

T0090 discovered gift card and multi-visit behavior without creating bookings, drafts, payments, redemptions, Aurora writes, AWS changes, or UI changes. Roller `POST /bookings/draft/costs` accepts `giftCards`, returns top-level `giftCards`, and returns `giftCardErrors` for an invalid gift card while keeping `bookingCosts.amountOwing` unchanged. Gift cards are separate from discount codes in the documented booking payload because Roller models `discounts` and `giftCards` as separate arrays, and Roller Help Center describes gift cards as stored value that can fully or partially cover purchases. Gift-card use is supported in Booking Costs/Create Draft Booking, while gift-card creation/administration stays in Venue Manager for this pilot. The initial Playground sampled `/data/giftcards` windows returned zero gift-card rows; after two Venue Manager fixtures were created and paid, `/data/giftcards` returned gift cards for booking references `5101043` and `5101044` with balances `500` and `100`. Booking costs applied both: the `100 kr` card reduced a `200 kr` quote to `amountOwing=100`, and the `500 kr` card reduced it to `amountOwing=0`. Payment Link should not be the primary path for gift cards/add-ons because it is a poorer PWA return flow and did not prove gift-card behavior. Product catalog contains `membership` products such as `10-Kort`, `20-Kort`, and `30-Kort`, and costs can be calculated for selling a `10-Kort`. Paid `10-Kort` booking `5101046` behaves like a membership product in booking detail, but Roller `GET /customers/4045520/multi-passes` returned zero balances and booking costs with the same guest email kept `amountOwing=200` with empty `multiPassAllocations`. Roller Help Center's beta multi-pass model says passes are linked to booking holder email/online account and automatically apply to eligible session pass products all-or-nothing in online/POS/Venue Manager carts. Current Playground `/products` returned no explicit beta `multipass` product type, so V1 must not show remaining multi-visit balance; a later ticket should validate whether Nacka membership/multi-visit codes can be sent as `discounts: [{ code }]` or another Roller-supported field.

T0093 validated the code-based Nacka `10-Kort` hypothesis with only safe Roller Playground `POST /bookings/draft/costs` calls. T0097 confirmed the same model after Gustav's product clarification and official Roller docs research. T0098 ran the approved controlled write smoke and created/published one dedicated Playground booking `5101114` using the masked code. The supported payload shape is `discounts: [{ code }]`. Invalid codes, the paid `10-Kort` booking reference, booking unique id, booking item id, and a normal paid entry ticket id all returned no amount reduction. The paid `10-Kort` ticket/code from booking `5101046` reduced a `200 kr` entry quote to `amountOwing=0`, created a paid no-owing booking in T0098, and still quoted as valid for up to ten entries after that write; Roller returned this as a normal discount with empty `multiPassAllocations`. V1 can support membership/`10-Kort` as code validation only, not remaining-balance display. A code should count as accepted only when Roller reduces `amountOwing` or returns a positive discount amount.

T0102 polishes the public phone buy-entry demo flow without changing backend or Roller behavior. After selecting a time and continuing, the phone app now shows one branded JumpYard availability-loading card instead of putting every time row in a loading state. Contact collection is now focused on first name, last name, email, and phone; gift card and Klippkort inputs moved to a collapsible payment-options section in the review/payment preparation step. Because those codes affect Roller quote/draft amounts, editing them requires a refreshed quote before the draft/payment button is enabled. The review summary now uses JumpYard icons for entry/add-on rows, labels the selected jump time explicitly, removes the duplicate grey footer total/time row, and the payment surface uses the JumpYard payment-card icon.
...
The post-T0110 Gustav review tickets are confirmed as implementation tickets, not followups. T0111 adds an explicit `Hämtar tillgängliga platser` loading state while the phone app fetches capacity/availability. T0112 fixes add-on price inconsistency across selection, summary, and payment preparation. T0113 removes hardcoded add-on prices and moves add-on pricing to Roller/JumpYard Cloud-derived data. T0114 cleans internal Roller product names into guest-friendly labels such as `Bryggkaffe`. T0115 fixes existing-booking add-on back navigation so summary/payment prep returns to add-on selection. T0116 relaxes unnecessary max-one limits for add-ons such as padlocks and SkyRider where multiple quantities are operationally reasonable. T0117 improves SkyRider information with height requirement, safety check/consent context, and recommendation to ride after jump time. T0118 replaces the gift-card/Klippkort `Uppdatera belopp` CTA with specific apply actions. T0119 improves gift-card/Klippkort input max length, valid/ready state, and feedback. T0120 shows human-readable staff dates such as `6 aug`. T0121 fixes the staff date-box layout. T0122 separates staff handout categories so staff can see what is handed out at check-in versus later collection, including padlocks, socks, visitor wristbands, SkyRider passes, and coffee.
...
T0127 completes the project context-hygiene foundation before the final Pelle/Anders rehearsal ticket. It is documentation/tooling-only: `CODEX_TASK.md` is moved from stale T0125 context through T0127 and then to `NO_ACTIVE_TICKET`, a project-context-hygiene skill and audit report define archive-first memory hygiene plus the English-default source-of-truth language policy, and validation is added so stale `CODEX_TASK.md` versus `REPO_CURRENT_STATE.md` active-ticket mismatches fail locally. Large historical sections are not moved in T0127, and T0126 remains the recommended next Pelle/Anders demo rehearsal.
...
| SMS/email readiness | Blocked pending AWS/user inputs | EventBridge booking-time messaging schedule exists but dev config keeps `confirmSend=false`. T0065 confirmed manual SMS delivery to the verified sandbox phone using the public Cloudflare check-in URL. T0067 confirmed SES real-email delivery acceptance for verified dev identity `love@wrlds.com`. T0068 unified booking-time planning for SMS and email. T0073 confirmed the unified due-message processor can send both SMS and email for one due booking to approved destinations. T0089 reconfirmed SNS SMS and SES remain sandboxed, with no production SMS sender identity, no SES production sender domain, and no unattended production sends enabled. | Before visitor-facing unattended sends: SMS production still needs AWS production access/sandbox exit, sender-display confirmation, final transactional copy, consent/support wording, expected volume, and any sender ID/provider setup. Email production still needs a verified domain or production from-address, recipient/sandbox policy, final sender/reply-to/consent text, DKIM/custom MAIL FROM where appropriate, and deliverability work. |
| Observability and alarms | Partial | T0060 added dashboard `jumpyard-check-in-dev-ops`, API/Lambda/DLQ/Roller API alarms, API Gateway access logs, and safe Roller outbound call metrics. T0061 added API throttling visibility and a 429 alarm. T0101 added `OPERATIONS_RUNBOOK.md` for dev signal meaning, AWS/Aurora checks, safe first actions, and escalation routing. | Add notification routing, Aurora health, scheduler-specific health, SMS failure metrics, webhook failure metrics, gift-card/Klippkort monitoring, and production thresholds. |
...
| Payment production readiness | In focus | Swish worked publicly in T0054; Pabel confirmed on 2026-06-01 that the Playground payment integration issue is fixed; T0075 confirmed card renders; T0076 and T0081 confirmed full card-paid new-booking flow reaches ready-for-staff; T0081 confirmed staff redeem from that flow; T0078 confirmed add-product payment before T0079 contact-removal; T0082 fixed the post-T0079 server-side contact-resolution blocker for no-customer add-product draft creation on JumpYard-created bookings. Existing JumpYard code uses Roller's approved payment package with response-only `paymentJwt`. | Keep the Roller package path. Before broader production readiness, run T0090-T0093 for gift-card payment and membership/multi-visit code behavior so those payment/entitlement cases are not discovered during go-live prep. Swish/Apple Pay visibility remains a Roller/Pabel follow-up. |

## Non-Goals For Current Ticket

- Do not implement gift-card UI in T0090.
- Do not implement multi-visit UI in T0090.
- Do not change JumpYard Cloud endpoints in T0090.
- Do not create Roller bookings, drafts, payments, or redemptions in T0090.
- Do not write to Aurora, deploy AWS changes, or change production credentials in T0090.
- Do not guess multi-visit behavior before Roller confirms the endpoint and payload.
...
| Does Roller Playground support an in-app payment flow from draft booking `paymentJwt`, including documented test/fake card numbers and any domain allow-listing requirements? | Determines whether F1 can complete payment inside the JumpYard PWA or must use a hosted fallback. Pabel confirmed the docs, `paymentSettings`, Adyen test card ending `1142`, and that `https://jumpyard-check-in.pages.dev` is allowlisted. T0054 confirmed Swish worked publicly, T0075 confirmed card rendering/payment, and T0076 confirmed the full card-paid new-booking flow. | `T0076` | `Answered for new-booking card flow` |
| How should gift cards and multi-visit passes work in buy-entry checkout? | Gift cards should reduce or cover the amount due during booking costs/draft/payment. Current Nacka `10-Kort` should use Roller membership/discount-code validation through `discounts: [{ code }]`, not beta multi-pass balance display. | `T0091/T0097/T0098/Josh/Joao/Pabel` | `Answered for V1: gift-card payload accepted and active full/partial fixtures work. T0097/T0098 confirmed Gustav's 10-Kort model as discount-code-style costs/draft validation with no balance/allocation exposed, so V1 must not show remaining visits.` |
...
## Archived REPO_CURRENT_STATE.md Known Issues And Questions

This section preserves the long known-issues and open-questions snapshot that T0128 removed from the active repo state. Current condensed questions remain in PROJECT_CONTEXT.md, FOLLOWUPS.md, and docs/roadmap/backlog.md.
