...
## Issue #227 Kiosk Terminal Payload Corrections

- 2026-08-15: PR #236 merged the server-owned terminal object contract as `506cbcb45ea20bfc1272db1e64c7bf4d35dec908`. Immutable release run `31876150698` built that exact commit; protected promotion run `31876392673` reviewed and applied a 202-to-202-resource plan changing only `BookingHandler5D1461BB` and `CDKMetadata`, with no migration, addition, or removal.
- 2026-08-15: AWS/CDK and Cloudflare deployment completed. The final workflow verification reported only the pre-existing `WRLDS:CostCenter` drift owned by issue #233. Independent readback confirmed account `376129878018`, region `eu-north-1`, stack `UPDATE_COMPLETE`, Booking Lambda `Active` with successful update, zero park-test alarms in `ALARM`, four empty queues, and the live `JumpYard` cost-center value preserved.
- 2026-08-15: After Love explicitly approved the caller identity, `/jumpyard-check-in-park-test/roller/credentials` was migrated in place from the legacy `paymentTerminals.primary` string to an object with the approved `deviceId`, preserved masked `terminalId`, tips disabled, and no amount. Guarded readback passed without printing either identifier. No new secret/resource, ROLLER draft, booking, publish, payment, refund, redemption, terminal action, guest send, or production change occurred; physical proof remains supervised and pending.
- 2026-08-15: ROLLER support confirmed that Create Draft requires `paymentTerminal` to be an object with `deviceId` and `terminalId`; `amount` is optional and defaults to the booking remainder, while `promptForTip` defaults to false. Focused validation proves legacy strings and incomplete objects fail closed, the normalized object forces tips off and omits a static amount, Booking Costs still excludes the field, Create Draft retains it, and both identifiers are redacted from provider errors. No operational identifier, draft, booking, payment, terminal activation, secret mutation, guest send, or production change was produced by this repository validation.
...
- 2026-07-14: Safe Roller Live Data API preflight proved provider modified-date windows must be at most one day. For `2026-07-13` through `2026-07-14`, aggregate totals were 217 booking items, 154 tickets, 145 payments, and 16 customers. No record contents, credentials, tokens, PII, or Roller write endpoint were printed or used.
- 2026-07-14: T0196 implemented an exact park-test/Live/Nacka `50871` approval guard, one-day provider splitting, one-request-per-second pacing, `Retry-After` plus bounded `429`/transient-`5xx` retry, page/window caps, reserved concurrency one, 30-day-past plus all-future visit filtering, related booking-reference/customer filtering, safe normalized upserts, structured metrics/receipts, and an operator that defaults to plan mode.
- 2026-07-14: Full `npm run validate` and `npm run infra:check` passed before rollout. Focused T0196, T0195 least-privilege, TypeScript, handler syntax, config-guard, operator self-test, synth/resource, and `git diff --check` checks passed during corrections. Final validation was rerun after code/docs completion.
- 2026-07-14: Reviewed CDK diff added exactly one CloudWatch freshness alarm and updated only the existing data-sync Lambda plus EventBridge rule. CloudFormation reached `UPDATE_COMPLETE` with 171 resources; data-sync readback was `Active`/`Successful`, reserved concurrency `1`, exact Live approval/venue/pacing/caps, and the rule enabled at `cron(0 2 * * ? *)`. Webhook processing and guest sends stayed disabled.
- 2026-07-14: The guarded backfill completed all 53 unique modified-date windows from `2025-07-14` through `2026-07-15`. Local mobile-network response loss produced safe idempotent duplicate success receipts; the operator was changed to fail closed on ambiguous AWS invocation errors and require receipt reconciliation before resume rather than blindly replaying provider reads.
- 2026-07-14: First related payment data exposed that PostgreSQL `ON CONFLICT` required target-column reads not present in the T0195 role. Migration `0013` granted only conflict-key/condition column reads, and ticket/payment/customer upserts were rewritten to bind parameters rather than require broad `EXCLUDED.*` target reads. The exact failed window then succeeded without table-wide related-data `SELECT`.
- 2026-07-14: A later window exposed a valid negative Roller payment representing refund/credit. Migration `0014` removed the obsolete nonnegative constraint; no value was discarded or made positive. The exact failed window then succeeded and aggregate readback confirmed 108 signed negative payment rows.
- 2026-07-14: Final Aurora aggregates were 6,174 bookings, 8,921 items, 6,662 tickets, 6,127 payments, and 983 guest profiles. All bookings grouped to Roller `live`, venue `50871`; zero were older than 30 days, 92 were for the current date, 120 were future, the minimum visit date was `2026-06-14`, and the maximum was `2026-12-30`. There were 54 initial/backfill success receipts and three retained failed-run audit rows before exact-window corrections; idempotent smoke/retry receipts added later without duplicate business rows.
- 2026-07-14: Scheduled-event smoke completed one-day provider windows and emitted success metrics. The freshness alarm reached `OK`. Controlled attempts while the reserved slot was occupied correctly triggered the existing data-sync throttle alarm; no parallel Lambda/provider execution occurred.
- 2026-07-14: Post-rollout CDK diff reported no differences. CloudFormation drift detection completed `IN_SYNC` with zero drifted resources. No Roller booking/payment/redeem/webhook write, webhook processing, guest message, lifecycle deletion, secret mutation, Cloudflare change, production resource, or broader venue was touched.

## T0195 Data Lifecycle, Least Privilege, And Recovery Repository Validation

- 2026-07-14: Love approved GitHub Issue #194 with the recommended policy for repository implementation and local/read-only proof. The policy deletes expired access/cache state within 24 hours, deletes or pseudonymizes visit/contact data at 30 days, deletes pseudonymous audit/run evidence and disabled-staff display PII at 90 days, keeps park-test logs/raw-bucket lifecycle at 30 days and Aurora backup/PITR at seven days, and continues to prohibit raw Roller/webhook payloads, payment JWTs, tokens, PINs, secrets, and unmasked credentials.
- 2026-07-14: Forward-only migrations `0010`-`0012`, the bounded lifecycle runner, aggregate database-backed evidence, per-handler restricted database roles, seven retained runtime/lifecycle secrets, expiry-aware handler reads, bounded secret/config caches, and 30-day managed log retention were implemented. Lifecycle apply binds the exact cluster, reference time, policy definition, counts, limits, mutation order, and receipt to one reviewed digest and transaction.
- 2026-07-14: PIN-pepper replacement is security-driven rather than scheduled. A guarded staged flow locks and recounts the identity/session set, clears old verifier material, revokes every staff session, commits a non-secret database version fence before Secrets Manager promotion, and rejects cached/stale administrator PIN writes with a retry response. Ordinary staff remain PIN-only; failed promotion stays fail-closed and is idempotently resumable without restoring old material.
- 2026-07-14: Phone buy-flow recovery uses a device-clock 12-hour expiry, immutable per-snapshot expiry, module-scoped monotonic deadline, minute observation checkpoints, active/cross-tab lifecycle cleanup, and fail-closed detected rollback before reuse. Validation does not claim JavaScript can delete while the browser is fully closed/offline or prove unobserved adversarial real elapsed time; that stricter guarantee would require server-trusted time.
- 2026-07-14: The recovery tool defaults to local plan mode and separately gates snapshot, restore, cleanup, and snapshot deletion. Restore resources are deterministic, tagged, encrypted, private, ingress-free, and never app-attached. Delayed cleanup accepts an older valid run id but re-verifies exact instance/cluster ARNs, engine, encryption, cluster membership, isolation group, non-public writer, and rehearsal tags immediately before deletion. Restore completion requires a lifecycle receipt that exactly matches the restored database's latest completed aggregate run row.
- 2026-07-14: Full `npm run validate` passed twice (138.8 and final-diff 126.1 seconds), and full `npm run infra:check` passed in 223.9 seconds, covering T0190-T0194 regressions, all five T0195 validators, every park-test synth profile, config guards, TypeScript, and operator self-tests. After the final cleanup ownership hardening, the affected infra TypeScript build and recovery validator passed again. Phone ESLint passed with zero errors and four existing `<img>` warnings; phone TypeScript and production static build passed.
- 2026-07-14: The reviewed park-test CDK diff completed successfully. Deployed template readback contained 154 resources; the synthesized target contained 170, with exactly 16 additions and zero logical-resource removals: seven retained database secrets, two log groups, two provider/provisioner Lambdas, two roles, two policies, and one custom resource. Existing handlers move from the administrator secret to dedicated principals and lose unused permissions; routes, public gates, production, Roller writes, guest sends, and webhook processing are unchanged.
- 2026-07-14: No migration, lifecycle mutation, secret change/promotion, deployment, snapshot, restore, cleanup, production resource operation, Roller write, payment, redemption, webhook processing, or guest message occurred. No local PostgreSQL runtime was available, so real migration/trigger/lock/action SQL remains deliberately unclaimed and must first be proven on an approved isolated Aurora restore. Every AWS/data/secret action still requires Love's separate post-review external-write approval. Nothing was committed or pushed.
...
- 2026-07-14: Migration `0009_staff_identity.sql` added stable staff identities, keyed PIN lookup plus domain-separated scrypt verification, hash-only opaque sessions, source/venue failed-login limiter state, individual invalidation, and credential-free lifecycle audit. Raw PINs, session tokens, source addresses, administrator passwords/TOTP, and Cognito bearer tokens are not persisted or logged.
- 2026-07-14: Local validation passed for PIN parsing and trivial/duplicate rejection, uniform failure/limiter behavior, session idle/absolute expiry, role/venue authorization, admin lifecycle, revoke/logout, named redeem audit before side effects, Cognito PKCE/callback cleanup, mobile source contracts, managed-login policy, ops CLI boundaries, synth/resource inventory, and dependency/migration checks. Both PIN and legacy staff bundles built without credentials or external writes.
- 2026-07-14: Reviewed AWS preflight confirmed account `376129878018`, region `eu-north-1`, owner `Love`, environment `park-test`, repository `wrlds-creations/jumpyard-check-in`, required tags, confidential/exportable data boundary, unassigned cost center, and containment rollback. The approved CDK diff added 20 Cognito/JWT and route/integration/permission resources, changed no production resource, and removed or replaced no existing resource.
- 2026-07-14: Migration `0009` was applied and the existing staff-auth secret was rotated in place to a generated server-only 64-character PIN pepper. CloudFormation reached `UPDATE_COMPLETE` with 154 resources and 26 API routes: six `AWS_IAM`, four Cognito `JWT`, and sixteen Lambda-protected `NONE`. The dedicated administrator pool kept MFA `ON`, Essentials tier, and deletion protection; all 17 alarms were `OK`, drift was `IN_SYNC`, and the post-deploy diff was clean.
- 2026-07-14: Love completed the first administrator password change, TOTP enrollment, and `/admin` login, then created an ordinary staff identity and confirmed PIN-only login. The approved eight-character administrator policy retained upper/lowercase and digit requirements, made symbols optional, and left TOTP required.
- 2026-07-14: Live negative and lifecycle proof passed without a Roller write: trivial PIN and forged session were rejected; duplicate PIN and reset behavior passed; logout returned `200`; disabled staff login returned `403`; re-enable restored `200` login and queue access; final logout returned `200`. Read-only Aurora aggregation found zero active local-PIN sessions after logout and complete create/reset/disable/enable/session/logout audit with required actor/environment/venue context and no forbidden credential, source-address, or email fields.
- 2026-07-14: API access logs exposed a second-login defect: first PIN login returned `200` and the immediate replacement returned `500` because two unordered data-modifying CTEs could collide with the one-active-session index. The correction uses an Aurora Data API transaction, identity `SELECT ... FOR UPDATE`, sequential revoke then insert, commit, and best-effort rollback. Focused tests covered two logins, old-token rejection, new-token heartbeat, transaction order, rollback, and credential-free SQL.
- 2026-07-14: The reviewed corrective AWS diff changed only the existing `SessionHandler` code asset. CloudFormation returned `UPDATE_COMPLETE`, Lambda readback was healthy, and the after-diff was empty. Love then completed three sequential live PIN logins; all returned `200`, every new token authorized the queue, and Aurora retained exactly one unrevoked session while predecessors were marked replaced.
- 2026-07-14: The same queue evidence exposed roughly 40 successful queue reads in a few seconds. Root cause was the queue effect depending on a mutable auth object while activity updates cloned that object and reset a local throttle. The final frontend uses a stable session key, ref-backed activity throttle, one in-flight plus pending refresh coordinator, versioned latest-query handling, synchronous query/selection refs, stale-response rejection, selection preservation, and explicit recovery when an old-session request overlaps a new PIN login.
- 2026-07-14: Admin ESLint, TypeScript `--noEmit`, the strengthened frontend validator, PIN and legacy production builds, and independent race review passed for request coalescing, fast session replacement, stale success/error suppression, duplicate debounce prevention, and selected-detail consistency. The exact park-test bundle then built with API `https://ij4rnaui2b.execute-api.eu-north-1.amazonaws.com`, PIN mode, Cognito domain, and public client `4cm36dkcrptlpq9j163q45ae56`.
- 2026-07-14: The final staff/admin build was direct-deployed to Cloudflare Pages at immutable URL `https://391533f3.jumpyard-checkin-admin-park-test.pages.dev`. Immutable and stable `/`, `/admin`, and `/auth/callback` resolved to HTTP `200`; the combined stable JavaScript contained the expected park-test API, PIN copy, Cognito domain, and public client. The Pages-only correction changed no AWS resource.
- 2026-07-14: Final independent review found that the initial staff redeem session read applied its booking-venue SQL filter only in historical Cognito mode, not active PIN mode. The nested trusted redeem path still blocked cross-venue Roller work, but a known foreign session id could reach an earlier blocked-session envelope. The correction always passes the authorized personal principal's venue and preserves null only for legacy dev principals without venue; the backend regression requires `AND booking.venue_id = :staffVenueId` with `50871` before any session response or Roller work.
- 2026-07-14: The reviewed corrective CDK diff changed only the existing `RedeemHandler` code asset. Deployment reached CloudFormation `UPDATE_COMPLETE` at `11:35:59Z`; Lambda readback was `Active` with `LastUpdateStatus=Successful`, and the post-deploy full-flow diff reported zero differences. No environment value, IAM policy, route, schema, secret, resource, production state, or Roller call/write changed.
- 2026-07-14: Final session review found a reset/login race between the unlocked PIN verification and locked session transaction. The locked identity read now requires the exact keyed lookup hash and scrypt verifier just verified. If an administrator reset changed either, login rolls back before revoke/insert, writes no success audit, returns uniform `403 staff_pin_invalid`, and exposes no token. The focused regression simulates reset between the two reads and confirms rollback-only behavior.
- 2026-07-14: The reviewed reset-race CDK diff changed only the existing `SessionHandler` code asset. Deployment reached CloudFormation `UPDATE_COMPLETE` at `11:42:52Z`; Lambda readback was `Active` with `LastUpdateStatus=Successful`, and the post-deploy full-flow diff reported zero differences. No environment value, IAM policy, route, schema, secret value, resource, production state, or Roller call/write changed.
- 2026-07-14: Love accepted the final deployed staff/admin result and explicitly chose to close T0194 without an additional post-fix manual traffic smoke. Closeout therefore relies on the earlier live functional/lifecycle proof plus automated request-stability assertions, exact-build verification, deployment readback, and independent code review; it does not claim post-fix live request-count evidence.
- 2026-07-14: T0194 closed without creating production resources, enabling webhook processing, importing broad Live data, sending automatic SMS/email, or performing a real Roller booking/payment/add-on/redeem write. `FU-050` moved to the done archive; T0195 remains planned and requires its own explanation and approval.

## T0193 Park-Test API Protection

- 2026-07-13: Love approved T0193 after the plain-language route-security explanation and explicitly required support for 120 guests over 20 minutes, including a 30-40 guest burst and shared park Wi-Fi.
- 2026-07-13: CDK now synthesizes one 21-route protection catalog with explicit handler/trust metadata, Gateway authorization, detailed metrics, rate/burst settings, and stage dependencies. Exact inventory validation found six `AWS_IAM` internal/legacy routes and 15 explicit `NONE` browser/Roller routes, 134 synthesized resources, and no WAF, CloudFront, domain, or authorizer resource.
- 2026-07-13: Guest protection validation passed for 32-byte opaque credentials, hash-only Aurora storage, booking binding, 60-minute maximum lifetime, 64-active steady-state soft cap without valid-token eviction, link-token reuse, atomic five-second cooldown, link/original expiry, wrong-class/expired/consumed rejection, session/add-on guards, safe envelopes, phone Bearer propagation, immediate URL scrub, and bounded transient retry.
- 2026-07-13: Service-auth validation passed for all five internal session-link/send routes, lower-level legacy redeem plan/write paths, staff list/detail/redeem missing/forged/expired tokens before downstream work, webhook missing/wrong/valid-disabled behavior, explicit IAM catalog, safe correlation ids, and no credential/payload log leakage.
- 2026-07-13: Payload validation passed plain and base64 oversized bodies before AWS/network work for lookup 8 KiB, booking 64 KiB, session 32 KiB, legacy/staff redeem 32 KiB, and webhook 256 KiB. Normal APIs return `413 payload_too_large`; webhook intake preserves safe HTTP `200` invalid/ignored acknowledgement.
- 2026-07-13: Deterministic capacity validation modeled 120 flows and 1,652 requests over 20 minutes, 702 requests in the busiest five minutes, and 40 devices/208 guest plus preflight requests in the first two seconds behind one IP with zero protection-caused modeled `429`. The abuse scenario throttled 30,912 of 41,400 requests without consuming unrelated route buckets.
- 2026-07-13: `npm run validate` and `npm run infra:check` passed. Infra check covered TypeScript build, all existing validation, all park-test synth profiles, T0193 API protection, payload, and service-auth validators. Existing CDK notice `37949` remained informational.
- 2026-07-13: Phone `npm run build` passed with `NEXT_PUBLIC_JUMPYARD_CLOUD_API_BASE_URL=https://ij4rnaui2b.execute-api.eu-north-1.amazonaws.com`; the static bundle contained that target. `npx tsc --noEmit` and scoped lint passed with zero errors and the single pre-existing `<img>` warning.
- 2026-07-13: AWS preflight reconfirmed account `376129878018`, region `eu-north-1`, `park-test`, stack `jumpyard-check-in-park-test-stack`, all ten WRLDS metadata values, 134-resource baseline, and `UPDATE_COMPLETE`. The reviewed diff changed only five Lambda code assets, 21 existing API routes, and the existing stage; it added, removed, or replaced no resource and did not change production.
- 2026-07-13: `npm --prefix infra run deploy:park-test-full-flow-rehearsal` completed successfully. CloudFormation returned `UPDATE_COMPLETE`; endpoint `https://ij4rnaui2b.execute-api.eu-north-1.amazonaws.com` remained unchanged.
- 2026-07-13: Deployed route readback returned exactly six `AWS_IAM` and 15 `NONE` routes. Stage readback returned default rate/burst `50/150` plus all 21 route-specific settings. Stack readback returned 134 resources and the unchanged required WRLDS tags.
- 2026-07-13: Runtime readback preserved full flow for Nacka `50871` and dates `2026-06-29` through `2026-09-30`, kept `ENABLE_ROLLER_WEBHOOK_PROCESSING=false`, and kept `ENABLE_GUEST_MESSAGE_SENDS=false`.
- 2026-07-13: Safe deployed smokes passed without real booking data or write effects: lookup CORS `204`; unsigned internal route `403`; signed internal call without valid app proof `401`; signed IAM plus valid app proof reached handler validation `400 identifier_required`; guest session without proof `401 guest_access_required`; staff list without token `403`; wrong staff passcode `403`; missing/wrong webhook token returned safe HTTP `200` ignored responses; valid webhook token reached `ignored_disabled`; oversized lookup returned `413 payload_too_large`.
- 2026-07-13: Rebuilt phone output was direct-deployed to Cloudflare Pages project `jumpyard-check-in-park-test`. Immutable deployment `https://c5b9d4db.jumpyard-check-in-park-test.pages.dev` and stable `https://jumpyard-check-in-park-test.pages.dev` both returned HTTP `200`; the fetched stable JavaScript contained the park-test API target, guest-auth handling, and `jy_token` cleanup path.
- 2026-07-13: Post-deploy `npm --prefix infra run diff:park-test-full-flow-rehearsal` reported no differences. Containment remains the normal closed park-test profile; reproducible prior-code artifact rollback remains T0198.
- 2026-07-13: T0193 closed with no new AWS resource/cost, production deploy, domain/DNS change, broad import, webhook processing, automatic SMS/email, personal staff identity, real Roller booking/payment/add-on/redeem write, or guest message send.
...
## T0195 Isolated Aurora Restore And Migration Checkpoint

- 2026-07-14: Explicit approval covered one encrypted park-test snapshot, one temporary isolated Aurora restore, and applying T0195 migrations to that isolated copy only. AWS identity matched account `376129878018` in `eu-north-1`; the source was available Aurora PostgreSQL `16.13`, encrypted, deletion-protected, Data API enabled, seven-day PITR, snapshot-tag-copy enabled, and matched all ten WRLDS tags.
- Snapshot `jy-park-test-prechange-20260714t154842z-ghunbv` became available without changing the source cluster. The first restore attempt stopped before cluster creation because AWS snapshot restore rejects `--enable-http-endpoint`; it left only zero-ingress security group `sg-0974372d5d5e9dce2` and no cluster/writer.
- The recovery tool was corrected to read AWS CLI `HttpEndpointEnabled`, enable Data API after restore, wait for the asynchronous change, and allow an exact same-run resume only after revalidating the isolated cluster, tags, security group, source, and absence of attached instances. Infra TypeScript build, recovery self-test, `validate-t0195-aurora-recovery.js`, and `git diff --check` passed after each correction.
- Restore run `20260714t155812z-ilzs0e` produced private cluster `jy-park-test-restore-20260714t155812z-ilzs0e-aurora`, private writer `jy-park-test-restore-20260714t155812z-ilzs0e-writer`, and zero-ingress security group `sg-04943ff0d41dcf891`. Readback confirmed encryption, Aurora PostgreSQL `16.13`, Data API, exact tags, one isolation group, `PubliclyAccessible=false`, no app attachment, and source cluster unchanged/available.
- Safe pre-migration aggregate readback found 25 bookings, 45 booking items, 59 tickets, zero guest profiles, two staff identities, and latest migration `0009`. The new run-bound migration mode accepted only the exact current external state file and isolated deterministic target; migrations `0010`-`0012` then applied successfully to the restore only.
- Read-only verification matched all 12 repository migration checksums across 24 tables with aggregate-set digest `5e3244b9e0fc5cd55beba1c66187f82fdeb753335f6b276d9bac4720777b7855`. Post-migration counts were unchanged, one active local-PIN identity retained usable version-1 verifier state, no lifecycle run existed, and no PIN/contact/booking values were printed.
- A read-only Aurora boundary query at reference time `2026-07-14T16:00:00Z` returned `future 2026-12-01=false`, `old 2026-06-01=true`, and `unknown-date=false` for lifecycle eligibility. This proves the visit clock on real Aurora without persisting a fixture.
- Lifecycle reapplication remained false and `rehearsalComplete=false` because the source had not yet received restricted-role password provisioning. No lifecycle mutation, source migration, stack deploy, secret change, PIN rotation/reset, Roller operation, app traffic, production change, or source-data deletion occurred during the restore proof.
- Love then explicitly approved cleanup. Guarded cleanup deleted the private writer, isolated restore cluster, and both zero-ingress rehearsal security groups; runs `20260714t155812z-ilzs0e` and `20260714t154842z-ghunbv` both reached `stage=cleaned`. The initial local waiter reached its 15-minute process limit while AWS still reported the writer as `deleting`; AWS completed the deletion, and guarded cleanup/readback then confirmed no `jy-park-test-restore-*` cluster, instance, or security group remained. The source was still available, encrypted, Data API enabled, deletion-protected, and on migration `0009`. Encrypted snapshot `jy-park-test-prechange-20260714t154842z-ghunbv` remained available under its separate deletion gate. No production resource was touched.
- Post-cleanup `npm run validate` passed all workflow, T0193, T0194, and T0195 checks. The unchanged infrastructure/code diff had already passed `npm run infra:check`; final `git diff --check` reported only accepted CRLF conversion notices.

## T0195 Park-Test Source Rollout

- 2026-07-14: Love explicitly approved the explained T0195 source rollout without lifecycle apply. Preflight matched account `376129878018`, region `eu-north-1`, all ten WRLDS tags, the 154-resource `UPDATE_COMPLETE` stack, Aurora PostgreSQL `16.13` with encryption/deletion protection/Data API/seven-day PITR, source migration `0009`, no temporary restore cluster, and the existing Nacka `50871` full-flow window through 2026-09-30 with webhook processing and guest sends off.
- Migration status/checksums showed only `0010`-`0012` pending. All three applied transactionally to source park-test. Post-migration status matched all 12 repository migrations; aggregate counts remained 25 bookings, 45 booking items, 59 tickets, zero guest profiles, and two staff identities, with zero lifecycle runs.
- Pre-deploy CDK diff contained exactly 16 additions: seven retained runtime/lifecycle secrets, two 30-day log groups, two database-role provisioner/provider Lambdas, two IAM roles, two IAM policies, and one custom resource. It tightened the six handler IAM/database-secret boundaries without removing/replacing resources or changing routes/gates.
- CDK deploy with `park-test-full-flow-rehearsal.json` reached `UPDATE_COMPLETE` and 170 resources. All six application Lambdas were `Active`/`Successful` and pointed to their own restricted database secret. Safe `SELECT current_user` connectivity passed for `jumpyard_{booking,data_sync,lookup,redeem,session,webhook,lifecycle}_runtime`; no secret value was printed.
- The read-only lifecycle dry-run used `/jumpyard-check-in-park-test/aurora/lifecycle`. It found 82 expired idempotency rows and planned a maximum batch of 25; all booking, item, ticket, payment, contact, staff, session, delivery, webhook, seed, link, event, and lifecycle-run actions returned zero. No apply/evidence row was created.
- Post-deploy readback found 26 routes (6 IAM, 4 JWT, 16 Lambda-protected), all 17 alarms `OK`, unchanged aggregate business data, migration `0012`, and zero lifecycle runs. CORS returned HTTP `204`; read-only Roller availability returned `available`, one slot, 12 products, and created no booking. Post-deploy CDK diff reported zero differences, and drift detection returned `IN_SYNC` with zero drifted resources. No lifecycle apply, PIN/secret rotation, webhook/message enablement, Roller write, Cloudflare change, or production action occurred.
- Final source-rollout documentation validation completed with `npm run validate` in 178.5 seconds; every workflow, safety-gate, T0193, T0194, and T0195 check passed.

## T0197 Roller Live Webhook Reconciliation Rollout

- 2026-07-15: Love approved issue #199 implementation, migrations `0015`-`0016`, the reviewed 16-resource CDK delta, park-test deployment, and bounded synthetic verification. Preflight matched account `376129878018`, region `eu-north-1`, all ten WRLDS tags, Roller Live, Nacka `50871`, and the exact approval `T0197_LIVE_WEBHOOK_RECONCILIATION_APPROVED`.
- Migration `0015` restricted webhook-role child cleanup to tickets/items. The first deployed synthetic intake correctly stopped with `webhook_persistence_error`; a direct restricted-role diagnostic found that PostgreSQL `ON CONFLICT DO NOTHING` also required read access to the `event_log.event_id` conflict key. Migration `0016` added only that column-level grant. No failed or stuck webhook event remained.
- The first authoritative reconciliation safely classified synthetic events `ignored_scope` because Roller booking detail did not include venue identity. The handler was tightened to cache and verify read-only Roller `/venues/me` as Live Nacka `50871` before filling a missing booking venue. The follow-up CDK diff changed only the intake and processor code assets, and deploy returned `UPDATE_COMPLETE`.
- Final stack readback found 187 resources: encrypted FIFO queue and DLQ, reserved-concurrency-one processor, batch-size-one partial-failure event mapping, five-attempt redrive, 12-minute visibility, five-minute recovery rule, and five new alarms. The public intake remained free of Roller REST calls. Roller webhook `1465` was enabled only for booking `Created`, `Updated`, and `Cancelled` with tickets, the exact endpoint, and `x-roller-apikey`.
- Deployed negative probes proved missing-token, malformed JSON, oversized payload, and unsupported event rejection. A valid synthetic event progressed `accepted -> processed`. An older `Created` arriving after a newer `Updated` also processed without regressing the authoritative snapshot; duplicate delivery returned `duplicate` and did not increase attempts.
- Guarded replay of an already processed event remained a no-op with unchanged attempts. A safe three-minute-old received fixture was recovered by the recovery path and processed once. A controlled invalid SQS message proved partial batch failure and the exact retry/DLQ contract; it intentionally exercised the processing-failure alarm. Main queue and DLQ visible/in-flight counts returned zero.
- Aurora aggregate verification ended with eight synthetic T0197 events: four early `ignored_scope`, four `processed`, zero `failed` or stuck, and zero retention violations. The latest target had one booking, one item, and two tickets. No natural Roller webhook arrived in the bounded observation window, so natural delivery remains a manual operational observation.
- Final CDK diff reported no differences and CloudFormation drift was `IN_SYNC` with zero drifted resources. No Roller booking/payment/redeem/webhook business write, guest send, secret mutation, lifecycle deletion, Cloudflare change, or production action occurred.

## Issue #212 Webhook Reconciliation Repair And Re-Promotion

- 2026-08-02: PR #213 merged as `f1743f1734d27bfaae7b4c0674a1587ad2d230f5`. Focused webhook/data-sync/migration tests, `npm run validate`, `npm run infra:check`, synth, and `git diff --check` passed. The implementation preserved signed booking-level amount owing, bounded automatic recovery below five attempts, emitted retry-exhaustion telemetry, added one alarm, and changed no guest-send or Roller-write gate.
- Immutable release run `30763166954` succeeded. Protected promotion run `30763549295` reviewed a 196-to-197-resource plan adding only `WebhookRetryExhaustedAlarmB94226A8`. It applied migration `0017`, reached `UPDATE_COMPLETE`, and deployed exact phone/admin outputs. Final verification exposed one API Gateway `$default` stage drift because the template's log-group ARN ended `:*` while the service normalized the same ARN without the suffix.
- Exact recovery then processed both known failed high-attempt rows and preserved one authoritative `amount_owing_cents = -371600` snapshot. Five DLQ messages were classified against five processed event rows before guarded 5/5 redrive; zero unknown messages, zero remaining failed/stuck rows, empty main/DLQ queues, and zero alarms in `ALARM` were confirmed. A new safe synthetic signal processed with event-id hash `cb703ca664c2d117`.
- PR #214 merged the canonical API access-log ARN and synth regression guard as `0aec9f8e6452692f18d920a093b6801a09b26708`. Full `npm run validate`, `npm run infra:check`, focused synth/build validation, read-only live CDK diff, and `git diff --check` passed. The live diff changed only `DefaultStage` and added or removed no resources.
- Release run `30765157585` built the exact merged SHA once. Protected re-promotion run `30765356271` reviewed a 197-to-197-resource plan changing only `DefaultStage`, kept migrations off, and passed exact-artifact deployment plus final `UPDATE_COMPLETE`, `IN_SYNC`, zero-alarm, empty-queue, migration-through-`0017`, Cloudflare commit, public phone/admin/admin-route, and Apple Pay association checks. Immutable phone `f27ad5d2.jumpyard-check-in-park-test.pages.dev` and admin `03706756.jumpyard-checkin-admin-park-test.pages.dev` returned HTTP 200 in a separate public readback. No rollback, Roller business write, guest email/SMS, secret output, arbitrary purge, lifecycle deletion, or production change occurred.
