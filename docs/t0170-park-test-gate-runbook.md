...
> T0190 update: repository code now treats the emergency stop as unconditional. Normal closed config keeps it on; reviewed active profiles release it explicitly and still require their narrow approval/date/venue rules. T0190 did not deploy this change, so the current AWS full-flow window remains on the earlier bypass model until a separately approved coherent code-and-config deploy.
...
The repository target model is:
...
|---|---|---|---|---|---|
| Emergency stop | Master stop for park-test handlers. No smoke, rehearsal, allowlist, or full-flow flag may bypass it. Missing/invalid runtime config also means stopped. | `safetyGates.emergencyStop` | `JUMPYARD_EMERGENCY_STOP` | `true` | Repository active profiles use `false` plus their narrow gates. Deployed AWS still uses the earlier `true` bypass model until a separately approved T0190 deploy. |
...
Under the T0190 source model, the closed profile keeps `emergencyStop=true`; every listed active profile sets it to `false` and still depends on its exact approval and scope fields. Setting it back to `true` safely stops the profile without allowing any bypass.
...
| Staff completes check-in | Staff auth, redeem/check-in writes | T0174 handout UI, T0175 payment readiness, T0176 frontend rehearsal, T0177 guest contact lookup, and T0178 UI/UX readiness before visitor use |
