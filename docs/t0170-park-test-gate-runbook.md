...
| Webhook processing | Whether incoming Roller Live webhooks write/enrich Aurora state. The webhook can exist while processing is disabled. | `safetyGates.rollerWebhookProcessingEnabled` | `ENABLE_ROLLER_WEBHOOK_PROCESSING` | `false` | Closed. T0172 decides off vs dry-run vs scoped processing before visitor testing. |
