...
- The complete Sprint 3 production target includes approved initial booking backfill, scheduled morning seed, Roller webhook processing/reconciliation, normalized Aurora state, and automatic SMS plus email links 30 minutes before selected booking time.
...
| Gate-02: SMS production access and consent inputs | `FU-045` | Provide expected volume/peak, countries, final transactional copy, consent and opt-out/support wording, sender goal, public URL, and approval for AWS support requests. | Love / AWS Support | Sandbox/approved test destinations only; no unattended production SMS. |
| Gate-03: SES production domain and sending access | `FU-063` | Choose final From domain/address, provide DNS access, obtain SES production access, and define deliverability/monitoring ownership. | Love / AWS Support / DNS owner | Verified sandbox identity only; no production guest email. |
