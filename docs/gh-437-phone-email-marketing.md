...
The worker requires `PHONE_EMAIL_MARKETING_PROVIDER_APPROVED=true`. Under D0224 only the Nacka release profile sets it, through the exact `safetyGates.phoneEmailMarketingDeliveryApproval` phrase; `scripts/validate-gh437-delivery-gate.test.js` keeps every other profile closed. The switch must not be promoted to the public phone origin before owned-address verification passes. The function-name references reuse existing invoke permission.
...
4. Confirm sender/audience scope (the current connection includes General), copy and evidence ownership. Love approved enabling the worker for Park verification (D0224); the owned-address results decide public promotion.
...
Issue #437 remains open. PR #439 (`Refs #437`) reached Park through release run 35862200997 and Park run 35862852072 with delivery disabled. The follow-up change (field-open reveal plus D0224 delivery approval) also ships to Park first. Love then runs two owned-address bookings on the Park phone origin: one without the choice and one with it, checking Klaviyo after payment. Public promotion of that same release follows only if both look right. No follow-up draft exists yet. Actual runs and readback belong in the rollout evidence.
