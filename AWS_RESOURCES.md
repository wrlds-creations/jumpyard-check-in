...
| `ij4rnaui2b` | API Gateway HTTP API | `park-test` | `eu-north-1` | `cdk` | Endpoint `https://ij4rnaui2b.execute-api.eu-north-1.amazonaws.com`; routes are deployed. T0156 CORS allows `https://jumpyard-check-in-park-test.pages.dev` and `https://jumpyard-checkin-admin-park-test.pages.dev`; Cloudflare project creation/update remains external to the local terminal. |
...
| `jumpyard-check-in-park-test-data-api-daily-sync` | EventBridge Rule | `park-test` | `eu-north-1` | `cdk` | Enabled at `cron(0 2 * * ? *)`; it must remain gated and perform no Live import until later tickets explicitly approve data sync behavior. |
...
| Status | T0150 foundation deployed; T0151 migrations through `0008` applied; T0155 Live webhook `1465` registered with processing disabled; T0156 park-test frontend target/CORS configured; Cloudflare project creation, frontend visitor traffic, payment, redeem, SMS, and email remain gated |
