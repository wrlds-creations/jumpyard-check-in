...
| `jumpyard-check-in-park-test-stack` | CloudFormation | `park-test` | `eu-north-1` | `cdk` | T0196 rollout reached `UPDATE_COMPLETE` with 171 resources. Post-deploy CDK diff has zero differences and drift detection is `IN_SYNC` with zero drifted resources. Stack ARN `arn:aws:cloudformation:eu-north-1:376129878018:stack/jumpyard-check-in-park-test-stack/159bdd20-6ae4-11f1-8f4c-069284999d99`. |
...
| `jumpyard-check-in-park-test-*` CloudWatch alarms | CloudWatch Alarms | `park-test` | `eu-north-1` | `cdk` | 18 alarms for API, Lambda, Roller API, DLQ, and T0196 booking-index freshness. The freshness alarm is `OK`; controlled concurrency smokes intentionally exercised and then cleared the existing data-sync throttle signal. |
...
| `jumpyard-check-in-park-test-stack-webhook` | Lambda | `park-test` | `eu-north-1` | `cdk` | Roller Live webhook `1465` remains disabled for processing; the handler now uses restricted role `jumpyard_webhook_runtime`. |
...
| `jumpyard-check-in-park-test-aurora` | Aurora PostgreSQL Serverless v2 | `park-test` | `eu-north-1` | `cdk` | Engine `aurora-postgresql 16.13`, database `jumpyard_cloud`, encrypted, deletion protection enabled, Data API enabled, 7-day backup retention, and snapshot tag copy enabled. Migrations `0001` through `0014` are applied. T0196 aggregate state is 6,174 bookings, 8,921 items, 6,662 tickets, 6,127 payments, and 983 guest profiles, with zero bookings beyond the 30-day past retention boundary and future visits through `2026-12-30`. |
...
T0007 created schema `jumpyard` in database `jumpyard_cloud`. Park-test is applied through migration `0014`; T0195 migrations `0010_data_lifecycle.sql`, `0011_runtime_database_roles.sql`, and `0012_staff_pin_pepper_version.sql`, plus T0196 migrations `0013_t0196_data_sync_conflict_keys.sql` and `0014_t0196_signed_payment_amounts.sql`, were applied during the approved 2026-07-14 rollouts.

| Table | Purpose |
|---|---|
| `schema_migrations` | Tracks applied SQL migrations and checksums. Retained for database integrity; park-test is applied through `0014 t0196 signed payment amounts`. |
...
| Status | T0150 foundation deployed; T0196 now has migrations through `0014` and 171 stack resources with restricted handler/lifecycle database identities plus the Live/Nacka booking-index freshness alarm. Live webhook `1465` remains registered but disabled; controlled Live flow smokes and the 53-window T0196 backfill passed; the Nacka/date full-flow window remains open through `2026-09-30`. Current API has 26 routes, freshness is `OK`, drift is `IN_SYNC`, and the post-deploy diff is clean. Administrator TOTP, staff create/reset/logout, repeated login, duplicate/negative cases, disable/re-enable, and named credential-free audit all pass. The T0195 lifecycle dry-run performed no deletion. Webhook processing, JumpYard-owned guest sends, broader venue scope, lifecycle apply, and production remain closed. |
