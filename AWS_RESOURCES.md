...
### Issue #264 Nacka Pilot-Production Role (Approved; Protected Rollout Pending)
...
Issue #264 creates no second AWS backend, renames no resource, changes no tag, and copies no operational data. Repository source adds `https://staff-checkin.jumpyard.se` to the existing API CORS contract and adds its callback/logout pair to the existing Cognito client contract. Those are source changes only until a selected immutable `main` artifact is reviewed and promoted through the protected Park workflow. Public phone/admin Cloudflare promotion is a separate protected, no-rebuild step. General guest messaging, kiosk, and future multi-park topology remain outside scope.
