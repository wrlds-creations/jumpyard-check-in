...
Selected/deployed template equality (`6bad56a633e87186155bfef5433a031d58cb7e32d041a670d77386936ae41d29`), `UPDATE_COMPLETE`, `IN_SYNC`, zero active alarms, empty queues and exact-SHA Park/public Pages checks passed. Independent readback matched all six deployed Booking Lambda files and 54 Park/public static responses. Same-artifact public promotion [35834923499](https://github.com/wrlds-creations/jumpyard-check-in/actions/runs/35834923499) succeeded after Park verification. Guest phone entry is required; the extra customer-match gate and manufactured phone are removed. Public phone lookup and placeholder SMS safeguards remain. Prior no-phone gate notes below are historical. No provider transaction, message, new resource, rollback or re-promotion occurred. [Full release evidence and remaining physical/provider acceptance](docs/gh409-no-phone-contact.md#protected-restoration-rollout--2026-09-23).

### Issue #409 No-Phone Contact Guard (Published 2026-09-15; Provider Gate Open)

[PR #422](https://github.com/wrlds-creations/jumpyard-check-in/pull/422) / `84b28a5` produced immutable [release 34984038346](https://github.com/wrlds-creations/jumpyard-check-in/actions/runs/34984038346), artifact `10402867670` (664 checksums independently verified). The reviewed [Park plan/deploy 34984709902](https://github.com/wrlds-creations/jumpyard-check-in/actions/runs/34984709902) changed only code/assets in the existing Booking, Lookup, Session, DataSync and two Webhook functions; all 17 deployed files independently match the artifact. Template `f1981878b54225e49cf944bfe128b792d71ec857cce2ff50c6b6b64ae9a2fbd9` retains 208 resources, 28 routes and migrations through `0023`; apply was false. Account/region, all ten WRLDS metadata values, IAM, schema, secrets, Nacka/date gates and general guest-send closure remain unchanged.

The initial run installed AWS/Pages successfully but its final alarm check failed on one ROLLER booking-detail 404 at 14:56:49 UTC. AWS remained `UPDATE_COMPLETE`/`IN_SYNC`; the alarm returned naturally to OK before delegated approval of the compatible newer [Park run 34985824066](https://github.com/wrlds-creations/jumpyard-check-in/actions/runs/34985824066), which succeeded. Its selected release `34984915362` / `02155e5` includes approved #421 quote reuse and has the identical backend/template, with no additional AWS changes. Template/drift, zero active alarms, empty queues, all 23 migrations, exact-SHA Pages and HTTP/Apple checks passed; 26 independent static responses match the combined artifact. No alarm threshold/state or deployment protection was changed. [Plans, hashes, publication evidence, rollback candidate and remaining provider/physical acceptance](docs/gh409-no-phone-contact.md#protected-rollout--2026-09-15).

The same artifact passed protected [public promotion 34987254267](https://github.com/wrlds-creations/jumpyard-check-in/actions/runs/34987254267) after Park success, independent readback and exact-plan approval. Exact-SHA Pages, public domains/CORS/Cognito/Apple checks and 26 independent public static responses passed. Paired kiosk deployment `eacfdd1d` / `4ac26eb` includes #100 and #111; it adds no AWS change. Prior release `34977478153` / `4bd7501` remains independently checksum-verified for rollback; no rollback or rebuild occurred. Unverifiable contacts still stop before payment, and the complete/atomic provider contact contract plus physical purchase acceptance remain open. No business transaction or guest send was performed.
...
Read-only preflight found six retained unresolved card-present drafts from August (four `unknown`, two `created`), with none in the preceding 24 hours. Their age is not proof of a safe retry. P400 lock migration and Kiosk 2 registration are deferred to reviewed reconciliation; no old attempt was edited or cleared. [Project #5](https://github.com/orgs/wrlds-creations/projects/5) holds the draft **Reconcile six legacy card-present attempts before P400 commissioning**. Direct configuration checks verify V210 resolution, two claim keys, exact P400 preservation and rejected local revocation. Live wrong-profile, wrong-venue and tampered-capability requests returned `409 kiosk_installation_not_authorized` before provider calls. APK/web installation, authenticated profile save and persistence/relaunch passed. The physical SEK 200 test observed one cancelled attempt, then Love's independent retry: one approved payment, confirmed booking, zero amount owing and one attached session. No additional infrastructure/configuration change, deployment or manual payment repair occurred. P400 physical acceptance remains open. [Physical evidence and attempt accounting](docs/gh-327-kiosk-terminal-binding.md#physical-v210-payment-evidence--2026-09-15).

### Issue #327 Kiosk Binding Backend (Deployed 2026-09-15; P400 Gate Open)

Reviewed [PR #411](https://github.com/wrlds-creations/jumpyard-check-in/pull/411) produced `ce7c795c1fd460936b552323805790d8bf657827`. Immutable [release 34966352071](https://github.com/wrlds-creations/jumpyard-check-in/actions/runs/34966352071), artifact `10394873499`, passed all 664 checksums and protected [Park deployment 34967225013](https://github.com/wrlds-creations/jumpyard-check-in/actions/runs/34967225013). The exact plan and an independent live-template comparison were reviewed before delegated approval under Love's deployment instruction. Only `BookingHandler5D1461BB.Properties.Code` changed; the 208 resources, IAM, schema, 28 routes, Nacka/date scope and gates remain. Migration apply was false; all 23 existing migrations were applied already.

Selected/deployed template equality, `IN_SYNC` drift, zero active alarms, empty queues, exact-SHA Park Pages and endpoint checks passed. Independent readback matched all six deployed Booking Lambda files to the selected artifact. Public phone/admin origins retain #343's `65efb38`; no public frontend promotion was needed. Live negative requests rejected malformed/unknown installations, incomplete add-on identity and kiosk identity on the phone channel before provider writes; the card-present attempt count for the preceding 24 hours remained zero. The existing ROLLER secret was inspected without exposing values and remains unchanged: the P400 mapping is preserved and no physical kiosk registration is provisioned yet. No AWS resource was added or removed. Existing account/region and all ten WRLDS metadata values were verified. Prior release `34963769097` / `65efb38` is retained and independently checksum-validated for protected rollback; no rollback or re-promotion occurred. [Rollout and remaining physical gates](docs/gh-327-kiosk-terminal-binding.md#protected-backend-rollout--2026-09-15).

### Issue #343 Localized Safety Media (Published 2026-09-15; No AWS Change)

Reviewed [PR #408](https://github.com/wrlds-creations/jumpyard-check-in/pull/408) produced `65efb3811a5ee226aa38e72b4449e72f985da09c`. Immutable [release 34963769097](https://github.com/wrlds-creations/jumpyard-check-in/actions/runs/34963769097), artifact `10394239520`, passed all 664 checksums, protected [Park 34964331709](https://github.com/wrlds-creations/jumpyard-check-in/actions/runs/34964331709) and same-artifact [public 34964817955](https://github.com/wrlds-creations/jumpyard-check-in/actions/runs/34964817955). The actual plans were reviewed before delegated approval under Love's explicit publication instruction; public approval followed Park success and independent readback. No rebuild occurred.

The selected/current/deployed template remains `e411cfc2b29802172b5f7f351f5a29fed23244d625788e59059eece5ce81e1a2`: 208 resources, no resource or section changes; CDK reported `no changes`. Migration apply was false, with all 23 migrations applied. Template/drift, alarms/queues, exact-SHA Pages, domains/CORS/Cognito/Apple checks passed. Existing account `376129878018`, `eu-north-1`, complete WRLDS metadata, Nacka scope/dates, IAM, schema, gates, backend and unchanged admin code remain. Only phone media/playback changed. Published #403 release `34341707774` / `bad087f` is unexpired and independently checksum-verified with the identical template for rollback. No rollback, re-promotion or business write occurred. [Exact approvals, media readback and manual limits](docs/gh-343-safety-video-recovery.md#localized-protected-rollout--2026-09-15).
...
The selected/deployed template hash is `84778ba5e89d189496317b65c3267e4747dc48574d1d38bd77bf55dc47de3cbd`. The plan added only the handout route/integration/Lambda permission (205 to 208 resources), its stage settings and session/redeem code. Migration `0021` applied once; all 21 live migration checksums match the artifact. Both Lambda packages independently matched all 10 source files; the actual restricted session role successfully read the 33-booking day board. All 661 artifact checksums, template equality, `IN_SYNC` drift, zero active alarms, empty queues, exact-SHA Pages, public HTTP/domain/CORS/Cognito/Apple checks and 30 independent static responses per lane (60 total) passed. [Approval and rollout evidence](docs/gh-345-staff-handout.md#protected-rollout--2026-09-08).

Existing Nacka account `376129878018`, `eu-north-1`, complete WRLDS metadata, venue/date gates and general guest-send closure remain unchanged. There is no database replacement, new runtime service or IAM action expansion. Counters retain non-guest aggregate high-water marks; claims/receipts follow guest/session retention. The pre-#345 rollback artifact remains available but requires reduced handout operation; migration/counters must never be reversed. No rollback, re-promotion, real customer admission/collection or guest send was performed. [Targets, privacy, migration, restore and rollback constraints](docs/gh-345-staff-handout.md).
...
### Issue #335 Park Alarm Routing (Deployed; Email Delivery Verified)

Reviewed [PR #381](https://github.com/wrlds-creations/jumpyard-check-in/pull/381) merged `1783cd468caa0198755841641fc3a55962bdeda0`, superseding closed PR #370 while preserving its source branch. Immutable [release 33876169856](https://github.com/wrlds-creations/jumpyard-check-in/actions/runs/33876169856), artifact `9938138226`, passed protected [Park 33876824492](https://github.com/wrlds-creations/jumpyard-check-in/actions/runs/33876824492) after exact plan review and delegated approval under Love's explicit instruction. It retains the preceding #339/#341 rollout.

The plan added one SNS topic, one email subscription and one sustained Roller-error alarm (202 to 205 resources), ALARM/OK actions on 27 existing alarms, a topic output and CDK analytics metadata. No resources were removed or replaced. The deployed canonical template hash is `45be018ca5b478d3b1e1962135df2370356044dce4a66d52e9f885af902c6abe`. Exact template, successful stack state, `IN_SYNC` drift, alarms, queues, migrations through `0020` (apply disabled) and Park frontend checks passed. Account `376129878018`, region `eu-north-1`, WRLDS metadata, business code, IAM, database, guest-send gates and Nacka scope are unchanged.

Live readback confirmed 31 alarms: 28 route ALARM and OK; three remain dashboard-only. The new sustained alarm uses `FILL(errors,0)` for three consecutive five-minute periods. The alias `aws-alarm@wrlds.com -> love@wrlds.com` is confirmed through authenticated SNS confirmation (`PendingConfirmation=false`, `ConfirmationWasAuthenticated=true`). One approved state-only `api-5xx` test sent ALARM at 13:18:20Z and naturally returned to OK at 13:18:58Z. Both SNS actions succeeded and the actual messages appeared in Love's inbox at 15:18 and 15:19 Europe/Stockholm. All 31 alarms were then OK. This proves the notification route, not incident recovery or response-time coverage. Cost drivers are one additional alarm and SNS publication/email usage; free-tier availability has not been established.

Love confirmed on 2026-09-04 that he is the sole responder and no other external monitoring exists. He owns first response using the acknowledgement/escalation runbook; the single recipient provides no backup, response-time or out-of-hours guarantee. This resolves the remaining #335 operational confirmation requirement. Rollback candidate is release `33875422657` / `0e04fb4f3a86d11366687e4aa7b4cd232d1fc4ce`, artifact `9937812376`, already verified in Park run `33875994276`. No rollback, re-promotion, public-origin promotion or guest message occurred for #335. Recreating the topic requires fresh subscription confirmation. [Hashes, delivery proof and operational confirmation](docs/gh-335-park-alarm-routing.md#protected-rollout-and-delivery-proof--2026-09-04).
...
| `jumpyard-check-in-park-test-stack` | CloudFormation | `park-test` | `eu-north-1` | `cdk` | #345 reached `UPDATE_COMPLETE` with 208 resources; selected/deployed template equality and `IN_SYNC` drift passed. T0197 established the earlier 187-resource baseline. Stack ARN `arn:aws:cloudformation:eu-north-1:376129878018:stack/jumpyard-check-in-park-test-stack/159bdd20-6ae4-11f1-8f4c-069284999d99`. |
| `ij4rnaui2b` | API Gateway HTTP API | `park-test` | `eu-north-1` | `cdk` | Endpoint unchanged. Current catalog has 28 explicit routes: six internal/legacy `AWS_IAM`, four administrator `JWT`, and eighteen browser/Roller `NONE` routes with caller-specific Lambda proof. Default rate/burst remains `50/150`; all 28 routes have settings and detailed metrics. #345 adds the personal-staff handout route with rate/burst `20/40`. CORS retains phone, admin, kiosk, and the controlled guest-alias origins. |
...
| `jumpyard-check-in-park-test-alarms` | SNS Topic | `park-test` | `eu-north-1` | `cdk` | Deployed #335 alarm channel. Default same-account policy, no CMK, alarm metadata only; one authenticated, confirmed email subscription to `aws-alarm@wrlds.com`, forwarding to `love@wrlds.com`. ALARM/OK receipt verified 2026-09-04. Recipients change only through reviewed config. |
...
| `jumpyard-check-in-park-test-*` CloudWatch alarms | CloudWatch Alarms | `park-test` | `eu-north-1` | `cdk` | 31 deployed alarms: 28 send ALARM and OK to the `jumpyard-check-in-park-test-alarms` topic, while `webhook-processor`/`data-sync` throttles and the single-period Roller error alarm stay dashboard-only; the new `roller-api-errors-sustained` alarm needs three consecutive failing five-minute periods. Previously 23 alarms for API, Lambda, Roller API, queue/DLQ state, T0196 booking-index freshness, and T0197 webhook processing. Seven webhook-prefixed alarms cover intake/worker error and throttle signals plus processing failure, queue age, and DLQ visibility. A controlled invalid-message proof intentionally exercised the processing-failure alarm; it and all six other webhook alarms subsequently returned `OK`. |
...
| `checkin_sessions` | Server-owned guest check-in session state, selected ticket ids, safety status, handoff status/code, expiry, and ready-for-staff state. #345 scopes new four-digit codes by park/allocation day while preserving legacy codes and stable session QR. |
| `handoff_day_counters` | Non-guest per-park/day allocation high-water marks. Transactional assignment, no same-day wrap, retained beyond guest deletion; never decrement during rollback/restore. |
| `staff_handout_claims` | Per-visit/area ownership, revisions, selected quantities and pending-operation identity; actor and visit locks prevent conflicting staff work. Guest/session deletion cascades apply. |
| `staff_handout_operations` | Durable collection intent and completed item/quantity/staff/time receipts, including partial café collection and admission recovery. Guest/session deletion cascades apply. |
