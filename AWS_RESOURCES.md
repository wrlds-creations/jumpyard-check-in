...
| `jumpyard-check-in-park-test-stack` | CloudFormation | `park-test` | `eu-north-1` | `cdk` | T0150 created the foundation; T0191 designated it as Sprint 3's sole pre-production foundation and T0192 qualified/hardened it. Current state is `UPDATE_COMPLETE`, 134 complete resources, and `IN_SYNC`; stack ARN `arn:aws:cloudformation:eu-north-1:376129878018:stack/jumpyard-check-in-park-test-stack/159bdd20-6ae4-11f1-8f4c-069284999d99`. |
| `ij4rnaui2b` | API Gateway HTTP API | `park-test` | `eu-north-1` | `cdk` | Endpoint `https://ij4rnaui2b.execute-api.eu-north-1.amazonaws.com`; routes are deployed. CORS preserves `https://jumpyard-check-in-park-test.pages.dev`, `https://jumpyard-checkin-admin-park-test.pages.dev`, and existing interface origin `https://jumpyard-check-in-kiosk.pages.dev`. Phone/admin Pages point at this API; kiosk implementation remains a separate workstream. |
...
T0146 defined the separate `park-test` environment contract, and T0150 deployed it. T0191 designated that foundation as Sprint 3's sole Live-backed pre-production environment; no parallel staging copy will be created. T0192 qualified and coherently hardened it without creating or replacing resources. The inventory above and the status row below record the current deployed posture. Production remains a separate post-GO T0205 environment, and every future AWS change still requires a scoped ticket and explicit approval.
...
| Status | T0150 foundation deployed; T0151 migrations through `0008` applied; T0155 Live webhook `1465` registered with processing disabled; T0156 park-test frontend target/CORS configured; Cloudflare park-test Pages projects created and direct-deployed; T0157 guarded Live quote/cost smoke passed; T0158 guarded Live draft smoke created one controlled draft; T0159 internal paid Live booking smoke passed; T0160 controlled Live existing-booking lookup smoke passed; T0161 Live catalog/index readiness passed; T0167 receipt email proof passed; T0168 add-on visibility fixed; T0169 post-payment sync passed; T0171/T0177 assisted Nacka/date-scoped booking reference/email/phone lookup is deployed for `2026-06-29` through `2026-09-30`; T0192 qualified the 134-resource foundation, deployed fail-closed stop/venue/request-item date gates, preserved phone/admin/kiosk CORS contracts, disabled the Playground-only daily sync rule, and confirmed zero drift; webhook processing, JumpYard-owned guest sends, broad same-day imports, and broader venue scope remain closed |
