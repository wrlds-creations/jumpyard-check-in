...
### Issue #220 Controlled `checkin.jumpyard.se` Test Alias (Deployed, Automated Proof Complete)
...
| Status | T0150 foundation deployed; T0201 now has migrations through `0017` and 199 stack resources. Live webhook `1465` feeds the dedicated FIFO worker/recovery path, and the repaired queue/DLQ state is empty. The Nacka/date full-flow window remains open through `2026-09-30`; broader venue scope remains closed. Current API has 26 routes, T0196 freshness is `OK`, drift is `IN_SYNC`, and the deployed template matches the selected release. Administrator TOTP and local-PIN staff flows remain validated. T0201 delivered exactly one automatic controlled email and was disarmed; the general guest-send gate remains false. The T0195 lifecycle dry-run performed no deletion. Roller business writes remain only within their prior approved gates; lifecycle apply, broad guest delivery, and production remain closed. |
