...
| `jumpyard-check-in-park-test-stack` | CloudFormation | `park-test` | `eu-north-1` | `cdk` | T0195 rollout reached `UPDATE_COMPLETE` with 170 resources. Post-deploy CDK diff has zero differences and drift detection is `IN_SYNC` with zero drifted resources. Stack ARN `arn:aws:cloudformation:eu-north-1:376129878018:stack/jumpyard-check-in-park-test-stack/159bdd20-6ae4-11f1-8f4c-069284999d99`. |
...
| `jumpyard-check-in-park-test-stack-data-sync` | Lambda | `park-test` | `eu-north-1` | `cdk` | Existing handler remains Playground-only with its park-test daily rule disabled, and now uses restricted role `jumpyard_data_sync_runtime`; T0196 owns approved Live backfill/morning seed. |
...
T0007 created schema `jumpyard` in database `jumpyard_cloud`. Park-test is applied through migration `0012`; T0195 migrations `0010_data_lifecycle.sql`, `0011_runtime_database_roles.sql`, and `0012_staff_pin_pepper_version.sql` were applied during the approved 2026-07-14 rollout.

| Table | Purpose |
|---|---|
| `schema_migrations` | Tracks applied SQL migrations and checksums. Retained for database integrity; park-test is applied through `0012 staff PIN pepper version`. |
...
| Status | T0150 foundation deployed; T0195 now has migrations through `0012` and 170 stack resources with restricted handler/lifecycle database identities. Live webhook `1465` remains registered but disabled; controlled Live flow smokes passed; the Nacka/date window remains open through `2026-09-30`. Current API has 26 routes, all 17 alarms are `OK`, drift is `IN_SYNC`, and the post-deploy diff is clean. Administrator TOTP, staff create/reset/logout, repeated login, duplicate/negative cases, disable/re-enable, and named credential-free audit all pass. The T0195 lifecycle dry-run found no booking/ticket/contact/staff candidates and performed no deletion. Webhook processing, JumpYard-owned guest sends, broad imports, broader venue scope, lifecycle apply, and production remain closed. |
