...
| `jumpyard-check-in-dev-booking-time-sms-schedule` | EventBridge Rule | `dev` | `eu-north-1` | `cdk` | Invokes `jumpyard-check-in-dev-stack-session` every 5 minutes for booking-time SMS processing in planning mode with `confirmSend=false`; T0049 adds explicit base URL and approval fields to the configured target payload for future confirmed-send gating. |
