...
### Issue #194 Repository Target (Not Deployed)

Love approved issue [#194](https://github.com/wrlds-creations/jumpyard-check-in/issues/194) for repository implementation, local/read-only validation, synth/diff, and non-mutating dry-run only. No T0195 migration, lifecycle apply, secret change, deploy, snapshot, restore, cleanup, or production resource operation has occurred. The deployed park-test stack therefore remains the 154-resource T0194 baseline until a separately reviewed external-write checkpoint.

| Area | Confirmed deployed park-test fact | Issue #194 repository target | External-write boundary |
|---|---|---|---|
| Aurora | PostgreSQL `16.13`, encrypted, deletion-protected, Data API enabled, cluster/writer retained, seven-day backup/PITR | Forward-only migrations `0010`-`0012`; bounded lifecycle runner and aggregate run evidence | Migrations and any dry-run against live Aurora or apply require a new approval; apply additionally deletes/anonymizes data |
| Database identities | All six handlers currently receive the Aurora administrator secret and broad Data API access | Six retained handler-specific runtime secrets/roles plus one retained lifecycle secret/role for every non-`dev` environment; administrator retained for migration/provisioning/recovery only | Database roles must exist before the stack switches handler secrets; deploy is not authorized yet |
| IAM | Existing handler roles include legacy S3/SQS/EventBridge access and session message permissions | Remove unused handler S3/SQS/EventBridge grants; grant only required Data API/secret operations; session receives SNS/SES only when existing guest-send gates enable them | Read-only synth/diff is allowed; CloudFormation update is separately gated |
| Existing secrets | Six stack-managed secrets, automatic rotation disabled | Apply retain-on-delete/update-replace consistently; bound runtime caches; keep provider changes manual | No current value is read, printed, rotated, promoted, or deleted in this checkpoint |
| Staff PIN pepper | Current `/jumpyard-check-in-park-test/staff/auth` is the server-only PIN pepper | Versioned secret contract plus security-driven `AWSPENDING` re-enrollment that clears old PIN material/revokes all sessions and advances a non-secret Aurora version fence before secret promotion | Stage, database re-enrollment, fence advance/promotion, or reset operation needs explicit security approval |
...
| Recovery | Automated backups exist; no temporary restore resource exists and no completed restore is claimed | Guarded unique snapshot and isolated private restore tooling with exact tags, no app attachment, aggregate verification, lifecycle reapplication, and database-backed receipt | Snapshot create/delete, restore create/cleanup, and any temporary resource mutation are four separately confirmed actions |

The planned non-`dev` CDK delta is exactly 16 resources above the deployed 154-resource baseline: seven retained secrets at `/<prefix>/aurora/runtime/{booking,data-sync,lookup,redeem,session,webhook}` and `/<prefix>/aurora/lifecycle`; two 30-day log groups; two Lambda functions for the deployment-time role provisioner/provider; two IAM roles; two IAM policies; and one custom resource. Handler secret/IAM rewiring adds no application route or gate. These resources do not exist in AWS yet. Restore resources are intentionally dynamic and temporary; none are represented as current inventory until an approved rehearsal creates them.
...
| `jumpyard-check-in-park-test-stack` | CloudFormation | `park-test` | `eu-north-1` | `cdk` | Current state is `UPDATE_COMPLETE` with 154 resources after T0194. Post-deploy CDK diff is clean and drift detection is `IN_SYNC` with zero drifted resources. Stack ARN `arn:aws:cloudformation:eu-north-1:376129878018:stack/jumpyard-check-in-park-test-stack/159bdd20-6ae4-11f1-8f4c-069284999d99`. |
| `ij4rnaui2b` | API Gateway HTTP API | `park-test` | `eu-north-1` | `cdk` | Endpoint unchanged. Current catalog has 26 explicit routes: six internal/legacy `AWS_IAM`, four administrator `JWT`, and sixteen browser/Roller `NONE` routes with caller-specific Lambda proof. Default rate/burst remains `50/150`; all 26 routes have settings and detailed metrics. CORS retains phone, admin, and kiosk origins. |
...
| `jumpyard-checkin-admin-park-test` | Cloudflare Pages | `park-test` | External | direct Wrangler deploy | Stable URL `https://jumpyard-checkin-admin-park-test.pages.dev`; final T0194 deployment `https://391533f3.jumpyard-checkin-admin-park-test.pages.dev` has active park-test API/PIN/Cognito configuration, phone font stack, black normal copy/active icons, mobile overflow safeguards, and coalesced request-stable queue refreshes. Direct deploy does not resolve the separate Git-trigger follow-up. |
...
| `jumpyard-check-in-park-test-stack-lookup` | Lambda | `park-test` | `eu-north-1` | `cdk` | Scoped reference/email/phone lookup remains open for Nacka through 2026-09-30. T0193 adds 8 KiB payload control and issues short-lived hash-only booking-bound guest proof; broad imports remain closed. |
| `jumpyard-check-in-park-test-stack-booking` | Lambda | `park-test` | `eu-north-1` | `cdk` | Current Nacka/date booking/payment/add-on gates remain. T0193 adds 64 KiB payload control and requires matching guest proof for existing-booking add-on quote/write before side effects. |
| `jumpyard-check-in-park-test-stack-redeem` | Lambda | `park-test` | `eu-north-1` | `cdk` | Staff redeem requires an active personal `staff_operator` opaque session, venue-filters the initial session read, rechecks venue/date/role before side effects, and records stable named actor audit. It has no PIN-pepper grant. Lower-level direct redeem still requires IAM plus its application token. |
| `jumpyard-check-in-park-test-stack-session` | Lambda | `park-test` | `eu-north-1` | `cdk` | Owns PIN lookup/slow verification, failed-login circuits, hash-only staff/admin sessions, admin lifecycle routes, guest proof, and handoff list/detail authorization. Repeated/concurrent logins for one PIN identity use a row-locked transaction that revokes the previous session before inserting its replacement. It is the only Lambda allowed to read the PIN pepper. Guest sends remain disabled. |
| `jumpyard-check-in-park-test-stack-webhook` | Lambda | `park-test` | `eu-north-1` | `cdk` | Roller Live webhook `1465` still targets the route. T0193 enforces a 256 KiB decoded ceiling and shared-token boundary while keeping safe HTTP `200` acknowledgements; processing remains disabled and valid intake returns `ignored_disabled`. |
| `jumpyard-check-in-park-test-stack-data-sync` | Lambda | `park-test` | `eu-north-1` | `cdk` | Existing handler is Playground-only. Its park-test daily rule is disabled; T0196 owns approved Live backfill and morning-seed behavior. |
...
| `jumpyard-check-in-park-test-aurora` | Aurora PostgreSQL Serverless v2 | `park-test` | `eu-north-1` | `cdk` | Engine `aurora-postgresql 16.13`, database `jumpyard_cloud`, encrypted, deletion protection enabled, Data API enabled, 7-day backup retention, and snapshot tag copy enabled. Migrations `0001` through `0009` are applied; personal identity, hash-only session, and hashed PIN-limiter tables exist alongside scoped Live smoke/lookup state, not a broad import. |
...
| `jumpyard-check-in-park-test-data-api-daily-sync` | EventBridge Rule | `park-test` | `eu-north-1` | `cdk` | Disabled. CDK enables the current Playground-only data-sync handler only when `roller.environment=playground`; T0196 owns any approved Live morning seed. |
...
| Roller Live webhook `1465` | Roller Webhooks API | `park-test`/Live | External | Roller | Posts booking `Created`, `Updated`, and `Cancelled` events with `tickets=true` to the park-test JumpYard Cloud webhook endpoint. Rollback endpoint: `https://api.roller.app/webhooks/1465`. Processing remains disabled in park-test. |
...
T0007 created schema `jumpyard` in database `jumpyard_cloud`. Park-test is applied through migration `0009`; T0195 migrations `0010_data_lifecycle.sql`, `0011_runtime_database_roles.sql`, and `0012_staff_pin_pepper_version.sql` exist only in the issue branch and have not been applied.

| Table | Purpose |
|---|---|
| `schema_migrations` | Tracks applied SQL migrations and checksums. Retained for database integrity; park-test is applied through `0009 staff identity`. |
...
| `staff_identities` | Named local-PIN staff and Cognito administrator registry, role/venue/revocation state, keyed PIN verifier material, and credential-free audit subject. T0195 adds deactivation/anonymization and non-secret PIN-pepper version/re-enrollment evidence when migration `0010`/`0012` is later applied. |
...
| `data_lifecycle_runs` | T0195 aggregate-only lifecycle plan/apply evidence. The table does not exist in park-test until migration `0010` is separately approved and applied. |
...
| Restricted diagnostic storage | S3 | Per environment | Private versioned storage with 30-day lifecycle; application policy prohibits persisting raw Roller/webhook payloads, payment JWTs, access tokens, PINs, secrets, or unmasked credentials. | Deployed to `dev` and existing `park-test`; no T0195 object or bucket change; production deferred to T0205 |
...
T0146 defined the separate `park-test` environment contract, and T0150 deployed it. T0191 designated that foundation as Sprint 3's sole Live-backed pre-production environment; no parallel staging copy will be created. T0192 qualified it, and T0193 deployed the production-intent route/application boundary without creating or replacing resources. The inventory and status row below record the current posture. Production remains a separate post-GO T0205 environment, and every future AWS change still requires a scoped ticket and explicit approval.
...
| Status | T0150 foundation deployed; migrations now run through `0009`; Live webhook `1465` remains registered but disabled; controlled Live flow smokes passed; the Nacka/date window remains open through `2026-09-30`; T0192 qualified the foundation, T0193 deployed route protection, and completed T0194 deployed the 154-resource personal PIN/Cognito identity model plus matching mobile/visual admin Pages build. Current API has 26 routes, all 17 alarms are `OK`, drift is `IN_SYNC`, and the identity after-diff is clean. Administrator TOTP, staff create/reset/logout, repeated login, duplicate/negative cases, disable/re-enable, and named credential-free audit all pass. Webhook processing, JumpYard-owned guest sends, broad imports, broader venue scope, and production remain closed. |
