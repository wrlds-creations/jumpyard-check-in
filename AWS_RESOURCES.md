...
### Issue #335 Park Alarm Routing (Implemented; Protected Rollout Pending)

Love approved [issue #335](https://github.com/wrlds-creations/jumpyard-check-in/issues/335) on 2026-09-03 after a read-only inventory: all 30 `jumpyard-check-in-park-test-*` CloudWatch alarms had zero alarm, OK or insufficient-data actions, and no SNS topic existed in `eu-north-1`. Thirty days of alarm history showed 71 `webhook-processor-lambda-throttles`, 39 `roller-api-errors`, 32 `api-5xx`, 7 `data-sync-lambda-errors` and 8 webhook/booking-index transitions to `ALARM` that reached nobody. Felix von Heland provides the WRLDS forwarding alias `aws-alarm@wrlds.com` as the verified recipient; SMS remains unavailable because the account is still in the SNS SMS sandbox.

The reviewed CDK change adds exactly three resources and no replacement: SNS topic `jumpyard-check-in-park-test-alarms` (default same-account policy, no CMK, alarm metadata only), one email subscription to the alias that delivers nothing until the recipient confirms, and the three-consecutive-period `jumpyard-check-in-park-test-roller-api-errors-sustained` alarm. 28 alarms gain `AlarmActions` and `OKActions` to the topic in place: API 5xx/high-4xx/throttled, Lambda errors for all seven handlers, Lambda throttles for the five handlers without reserved concurrency, both DLQs, webhook queue age/processing failure/retry exhausted, booking-index freshness, the six SES alarms and the sustained Roller alarm. `webhook-processor-lambda-throttles`, `data-sync-lambda-throttles` and the single-period `roller-api-errors` alarm stay dashboard-only because they fire during normal serialized operation or on single rejected Roller requests. Modelled on the last 30 days, the routed set would have produced 48 ALARM emails (about 1.6 per day, zero on 15 of 30 days) plus the matching OK emails; the sustained Roller alarm would have fired once instead of 39 times.

Local validation on the `codex/gh-335-park-alarm-routing` branch: infra type-check, release-profile synth (205 resources, 31 alarms), `validate:config-guards`, `validate:park-test-synth`, the T0193/T0194 template validators (resource count 202 to 205) and the new `validate:gh335-alarm-routing` all pass; a read-only `cdk diff` against the deployed stack listed only the three additions and the 28 in-place alarm action updates. No AWS resource has been created or changed. Rollout follows the T0198 path: merge, immutable release, protected Park plan and approval, then the alias owner confirms the SNS subscription email, and one controlled `set-alarm-state` notification test runs only in a separately approved window before readback of topic, subscription status and alarm actions. Rollback is the previous release artifact, which removes the topic and its subscription; a later re-creation needs a new confirmation. Estimated cost is within the SNS free tier.
...
| `jumpyard-check-in-park-test-alarms` | SNS Topic | `park-test` | `eu-north-1` | `cdk` | Issue #335 alarm notification channel (pending protected rollout). Default same-account policy, no CMK, alarm metadata only; one email subscription to the WRLDS forwarding alias `aws-alarm@wrlds.com` that must be confirmed by the recipient before delivery. Recipients change only through reviewed config. |
