...
| `jumpyard-check-in-park-test-stack` | CloudFormation | `park-test` | `eu-north-1` | `cdk` | T0150 created the foundation, T0192 qualified it, and T0193 deployed in-place API protection. Current state is `UPDATE_COMPLETE` with 134 resources and a clean post-T0193 CDK diff; T0192's last explicit drift detection was `IN_SYNC`. Stack ARN `arn:aws:cloudformation:eu-north-1:376129878018:stack/jumpyard-check-in-park-test-stack/159bdd20-6ae4-11f1-8f4c-069284999d99`. |
| `ij4rnaui2b` | API Gateway HTTP API | `park-test` | `eu-north-1` | `cdk` | Endpoint unchanged. T0193 readback: 21 explicit routes, six internal/legacy `AWS_IAM` and 15 browser/Roller `NONE` with caller-specific Lambda proof; default rate/burst `50/150` plus 21 route overrides and detailed metrics. CORS retains phone, admin, and kiosk interface origins; no WAF/domain/authorizer resource was added. |
...
| `jumpyard-check-in-park-test-stack-redeem` | Lambda | `park-test` | `eu-north-1` | `cdk` | Staff redeem remains staff-authenticated and venue/date bounded. T0193 adds shared 32 KiB decoded parsing, expired-token rejection before downstream work, and IAM plus app token for lower-level direct redeem. |
| `jumpyard-check-in-park-test-stack-session` | Lambda | `park-test` | `eu-north-1` | `cdk` | T0193 enforces booking/session-bound guest proof, link reuse/cooldown, 32 KiB bodies, staff token validation, safe correlation/logging, and IAM on five internal link/send routes. Guest sends remain disabled. |
...
| `/jumpyard-check-in-park-test/staff/auth` | Secrets Manager | `park-test` | `eu-north-1` | `cdk` | Current shared park-test staff-auth container; phone/admin full-flow traffic is connected, while T0194 owns personal production-intent identity. |
...
| Status | T0150 foundation deployed; migrations through `0008`; Live webhook `1465` registered but processing disabled; park-test phone/admin targets and CORS configured; controlled Live quote/draft/payment/lookup/add-on/redeem/receipt/sync smokes passed; Nacka/date-scoped reference/email/phone lookup is open for `2026-06-29` through `2026-09-30`; T0192 qualified the 134-resource foundation and disabled the Playground-only daily sync; T0193 deployed six IAM-plus-app-token internal/legacy locks, guest/staff/webhook proof, payload ceilings, and shared-IP-safe route settings with no new resource, then deployed the matching phone build and confirmed a clean diff. Webhook processing, JumpYard-owned guest sends, broad same-day imports, broader venue scope, and production remain closed. |
