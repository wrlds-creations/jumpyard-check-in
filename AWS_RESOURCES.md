...
### Issue #216 T0201 Controlled T-30 Email (Approved, Not Deployed)

Love approved issue [#216](https://github.com/wrlds-creations/jumpyard-check-in/issues/216) for one automatic park-test email tied to one separately agreed Roller Live booking, start time, and retained booking email. The implementation is being prepared on `codex/gh-216-t30-email`; no AWS resource, secret value, schedule, IAM permission, SES setting, email, or production state has changed yet. The live application stack remains the deployed 197-resource #212 baseline until a reviewed PR is merged and its immutable release receives protected promotion approval. The current local full-flow assembly synthesizes 199 resources; the protected comparison against the live processed template remains authoritative for the eventual rollout delta.

The reviewed source delta plans one five-minute email-only EventBridge rule plus its Lambda invoke permission, conditional session-to-lookup invocation for a final Roller Live read, and session SES permission restricted to the exact `jumpyard.se` identity, `nackaforum@jumpyard.se` From address, and `jumpyard-check-in-park-test-email` configuration-set resource. SNS permission is not added. The existing retained `/jumpyard-check-in-park-test/checkin-links/dev-token` JSON secret carries the nested `t0201Control` object so rollback/re-promotion cannot orphan a newly named secret; its existing token and other keys are preserved during arming/disarm. Normal `park-test.json` stays closed; the release profile can run only with `guestMessagingSendsEnabled=false` and control state that contains no raw booking identifier or email address. The control requires hashes for one booking and normalized retained email plus Nacka `50871`, exact start time, approval, schema, and nonce. Missing, unarmed, stale, early, ambiguous, or mismatched state makes no SES call.

The steady source design introduces no new paid secret and up to 288 short scheduler invocations per day; measured cost remains a follow-up before broader rollout. The future real proof requires a separate exact-booking confirmation, redacted secret arming, protected release/promotion evidence, and post-result disarm. `checkin.jumpyard.se`, Apple Pay/payment-domain validation, SMS, production, and broad booking delivery remain outside #216.
...
| `jumpyard-check-in-park-test-stack-booking` | Lambda | `park-test` | `eu-north-1` | `cdk` | Current Nacka/date booking/payment/add-on gates remain and the handler now uses restricted role `jumpyard_booking_runtime`. |
...
| Table | Purpose |
|---|---|
| `schema_migrations` | Tracks applied SQL migrations and checksums. Retained for database integrity; park-test is applied through `0016 t0197 event log conflict key`. |
...
| Roller rate-limit control | SQS plus DLQ | Per environment | Serialize Roller operations and provide dead-letter handling. | General queue deployed to `dev` and park-test; T0197 adds a dedicated encrypted FIFO webhook queue/DLQ in park-test; production deferred to T0205 |
...
| `jumpyard-check-in-park-test-email` | Amazon SES configuration set | existing `park-test` | Fail-closed TLS-required sending boundary with bounce/complaint suppression and dedicated event telemetry. | Deployed; final `SendingEnabled=false`; controlled proof returned to the IaC state |
| Guest email delivery/reputation monitoring | SES event destination and CloudWatch | existing `park-test` | Send/delivery/bounce/complaint/reject/rendering-failure metrics plus bounded event and account-health alarms without recipient/body/token dimensions. | Deployed; controlled proofs reported three sends/deliveries and zero failure events |
...
| Status | T0150 foundation deployed; T0197 now has migrations through `0016` and 187 stack resources. Live webhook `1465` is enabled for the exact booking-event boundary and feeds a dedicated FIFO worker/recovery path; controlled negative, valid, out-of-order, duplicate, replay, recovery, retention, and DLQ-contract checks passed with empty queues and no failed/stuck event. The Nacka/date full-flow window remains open through `2026-09-30`; broader venue scope remains closed. Current API has 26 routes, T0196 freshness is `OK`, drift is `IN_SYNC`, and the post-deploy diff is clean. Administrator TOTP and local-PIN staff flows remain validated. The T0195 lifecycle dry-run performed no deletion. Roller business writes remain only within their prior approved gates; JumpYard-owned guest sends, lifecycle apply, and production remain closed. |
