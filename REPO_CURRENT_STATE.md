...
- Date: 2026-08-18
- Latest merged product baseline: `67e6ffa`; PR #259 makes authoritative webhook enrichment reconcile onto an existing provisional kiosk item by `booking_item_id` without changing its local row key.
- Latest deployed product baseline: protected run `32136044606` re-promoted immutable release `67e6ffa` after a 202-to-202-resource plan with no additions, removals, or changes. Exact AWS/Cloudflare release, `UPDATE_COMPLETE`, `IN_SYNC`, zero alarms, empty queues, public phone/admin/Apple Pay checks, and migrations through `0020` all passed.
...
- Product/runtime state: park-test has 202 resources, migrations through `0020`, and 27 API routes. Phone availability and the Park bundle expose Weekday Combo `1242135`/`1242136` and omit the retired ComboDeal. Handoff auto-refreshes a syncing provisional booking and the webhook worker now enriches that same item with the authoritative Roller product instead of leaving it as generic `Övrigt`. The controlled check-in alias remains park-test-backed; the T-30 control is disarmed and the general send gate is false.
...
- Routine park-test deployment is GitHub-native. Issue #257 immutable release run `32133478654` produced `67e6ffa`; protected run `32136044606` re-promoted it after an exact no-change plan and passed the full AWS, Cloudflare, public-route, queue, alarm, drift, and migration verifier.
