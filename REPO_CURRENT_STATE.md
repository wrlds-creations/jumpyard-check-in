...
- Date: 2026-08-19
- Latest merged product baseline: `96022c8`; PR #275 keeps the mobile staff redeem confirmation visible until an explicit next action, even after the redeemed session leaves the refreshed queue.
- Latest deployed baseline: release run `32241835729`, Park run `32242333784`, and public run `32242663090` promoted `96022c8`; exact release, AWS/Cloudflare, drift, alarms, queues, migrations, phone/staff, Cognito, and Apple association checks passed.
...
- Product/runtime state: park-test has 202 resources, migrations through `0020`, and 27 API routes. Both public origins run `96022c8`; guest loaded live Roller capacity and staff loaded PIN/admin with the correct Cognito callback. Real public purchase and personal-PIN redemption complete without the prior logout/retry loop. A fresh Motorola proof kept the mobile success view visible for 15 seconds after the queue became empty, then returned explicitly without logout. Phone/Park expose Weekday Combo `1242135`/`1242136`, not retired ComboDeal. Visible authenticated Handoff refreshes every five seconds, preserves compact pending/selected detail, and enables `Slutför` only after authoritative tickets. T-30 is disarmed and the general send gate is false.
...
- Routine Park/public deployment is GitHub-native. Release `32241835729`, Park run `32242333784`, and public run `32242663090` promoted `96022c8` with exact verification; direct Cloudflare Git is off on all four retained projects. Redeem falls back to authenticated `/venues/me` context when booking detail omits `venueId`, authentication survives non-authentication failures, and the mobile success view waits for an explicit next action.
