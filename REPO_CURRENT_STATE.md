...
- Date: 2026-08-24
- Latest merged/deployed product baseline: `80ffb5e`; PRs #295/#297 isolate daily product-cache commits, upsert stable booking-item ids, and serve current Live add-on prices from the 24-hour cache. Release `32718610270` and Park run `32719040785` deployed it; the global verifier stopped only on the historical booking-index alarm.
...
- The deployed backend is `80ffb5e`. Its successful manual sync emitted `BookingIndexSyncSuccess=1`; the six-hour historical alarm had not reevaluated at closeout and was not reset or suppressed.
