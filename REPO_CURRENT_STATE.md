...
- Date: 2026-08-18
- Latest merged product baseline: `2f23725`; PR #265 keeps the authenticated Handoff queue live with a visibility-aware five-second refresh, compact pending sync, and authoritative redemption gating.
- Latest deployed product baseline: protected run `32142323722` promoted immutable release `2f23725` after a 202-to-202-resource plan with no additions, removals, or changes. Exact AWS/Cloudflare release, `UPDATE_COMPLETE`, `IN_SYNC`, zero alarms, empty queues, public phone/admin checks, and migrations through `0020` all passed.
...
- Product/runtime state: park-test has 202 resources, migrations through `0020`, and 27 API routes. Phone availability and the Park bundle expose Weekday Combo `1242135`/`1242136` and omit the retired ComboDeal. While authenticated and visible, Handoff refreshes its queue every five seconds without a full loading flash, refreshes immediately after returning to the foreground, keeps selected provisional detail live, and enables `Slutför` only after authoritative ROLLER tickets arrive. The webhook worker enriches that same item with the authoritative ROLLER product instead of leaving it as generic `Övrigt`. The controlled check-in alias remains park-test-backed; the T-30 control is disarmed and the general send gate is false.
...
- Routine park-test deployment is GitHub-native. Issue #266 immutable release run `32141895289` produced `2f23725`; protected run `32142323722` promoted it after an exact no-change plan and passed the full AWS, Cloudflare, public-route, queue, alarm, drift, and migration verifier.
