...
- Date: 2026-08-20
- Latest merged product baseline: `9631bf3`; PR #280 reconciles kiosk-provisional booking items during the mandatory final ROLLER redeem refresh without replacing their persisted keys.
- Latest deployed baseline: release `32350834677` and Park run `32351230117` deployed `9631bf3`; public frontends remain on compatible `96022c8`. Redeem is exact/zero-diff after deployment, while an unapproved Project investigation owns the unnamed failing post-check.
...
- Product/runtime state: Park has 202 resources, migrations through `0020`, and 27 routes. Public frontends `96022c8` target backend `9631bf3`; purchase, PIN redemption, persistent success, and kiosk-created booking redemption are Motorola-verified. Phone/Park expose Weekday Combo `1242135`/`1242136`. Handoff refreshes every five seconds and remains fail-closed until authoritative tickets. T-30 and general sends are off.
...
- The 202-resource/27-route stack's deployed `9631bf3` template matches the release exactly. Prior `96022c8` evidence proved sync/alarms/queues; the unnamed failing `9631bf3` post-check remains unconfirmed and separately tracked.
- GitHub-native release `32350834677` and Park run `32351230117` deployed backend `9631bf3`; public run `32242663090` remains the latest frontend promotion at `96022c8`. Redeem handles missing venue, preserves staff state and kiosk item keys, and keeps success visible until staff acts.
