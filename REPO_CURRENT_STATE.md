...
- Date: 2026-08-18
- Latest merged product baseline: `fc8e1c4`; PR #268 adopts Park as Nacka pilot production and protects Park/public frontend promotion.
- Latest deployed baseline: Park run `32146904664` and public run `32147234728` promoted `fc8e1c4`; exact release, AWS/Cloudflare, drift, alarms, queues, migrations, phone/staff, Cognito, and Apple association checks passed.
...
- Product/runtime state: park-test has 202 resources, migrations through `0020`, and 27 API routes. Both public origins run `fc8e1c4`; guest loaded live Roller capacity and staff loaded PIN/admin with the correct Cognito callback. Phone/Park expose Weekday Combo `1242135`/`1242136`, not retired ComboDeal. Visible authenticated Handoff refreshes every five seconds, preserves compact pending/selected detail, and enables `Slutför` only after authoritative tickets. T-30 is disarmed and the general send gate is false.
...
- Issue #264 makes technical `park-test` Nacka's sharp pilot backend without changing its AWS/data identity; protected run `32147234728` promoted both public origins. Multi-park remains separate.
...
- Routine Park/public deployment is GitHub-native. Release `32145647163`, Park run `32146904664`, and public run `32147234728` promoted `fc8e1c4` with exact verification; direct Cloudflare Git is off on all four retained projects.
