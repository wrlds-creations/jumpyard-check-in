...
- Date: 2026-08-07
- Latest merged product baseline: `e84df51` after PR #225 added the shared ROLLER card-present kiosk backend contract; the controlled guest-domain changes from PRs #221-#222 remain intact.
- Latest deployed product baseline: protected run `31164423038` promoted immutable release `e84df51` to the normal park-test stack, phone, and admin targets. The separate controlled `https://checkin.jumpyard.se` alias still serves selected phone artifact `9ffe379` against the same API. Issue #216 controlled automatic T-30 email remains deployed and disarmed.
