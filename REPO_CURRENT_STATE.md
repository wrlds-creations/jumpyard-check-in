...
- Date: 2026-08-20
- Latest merged product baseline: `31e2c54`; PR #286 routes kiosk add-ons for an existing booking through the configured card-present terminal while preserving the phone/Park ecommerce path.
- Latest deployed baseline: release `32372219796` and protected Park run `32372746116` deployed backend `31e2c54` and its compatible public outputs. CloudFormation is `UPDATE_COMPLETE`; the combined verifier stopped only on the pre-existing booking-index-stale alarm after the deployment steps succeeded.
...
- The 202-resource/27-route stack's deployed backend is `31e2c54`. The protected run stopped after deployment because `jumpyard-check-in-park-test-booking-index-stale` was already in `ALARM`; no alarm was reset or suppressed.
