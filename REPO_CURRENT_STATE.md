...
- Date: 2026-08-18
- Latest merged product baseline: `67e6ffa`; PR #259 contains the #257 provisional-item webhook fix, pending its own protected deployment and recovery.
- Latest deployed product baseline: protected run `32133387124` promoted release `2c383cc`. AWS and phone/admin Pages deployment succeeded; its final verifier stopped on the independent #257 webhook alarms and backlog.
...
- Product/runtime state: park-test has 202 resources, migrations through `0020`, and 27 API routes. Phone availability and the Park bundle now expose Weekday Combo `1242135`/`1242136` and omit the retired ComboDeal. The #257 webhook backlog still requires its separate rollout. The controlled check-in alias remains park-test-backed; the T-30 control is disarmed and the general send gate is false.
...
- The stack has 202 resources and 27 routes (6 IAM, 4 JWT, 17 Lambda-protected). Selected and deployed templates match and CloudFormation drift is `IN_SYNC`. Four alarms plus messages in the webhook queue/DLQ remain owned by #257; #260 did not purge or alter them.
