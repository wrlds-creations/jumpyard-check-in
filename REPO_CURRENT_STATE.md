...
- Date: 2026-08-24
- Latest merged/deployed backend baseline: `3e1899a`; PR #301 makes guest session resume return the original booking plus approved linked add-ons without exposing internal linkage or duplicating provisional/authoritative rows. Release `32726906347` and protected Park run `32727410550` deployed it with only `SessionHandler3CE835D7` changed and all deployment verification green. PRs #295/#297 remain the deployed product-cache/pricing baseline.
...
- Product/runtime state: Park has 202 resources, migrations through `0020`, and 27 routes. A full daily sync succeeded with 512 products and 2,024 retained booking-item upserts without duplicates. Cached public prices are socks/water 49 kr, lock 45 kr, and coffee 35 kr. Purchase, linked add-on Handoff, PIN redemption, persistent success, kiosk redemption, and late Handoff attachment are device-verified. Guest resume now includes approved linked add-ons before and after ROLLER readback; supervised full-restart/original-QR proof remains open in kiosk issue #48. Phone/Park expose Weekday Combo `1242135`/`1242136`; guest sends are off.
...
- The deployed backend is `3e1899a`. Protected run `32727410550` completed with exact template equality, `IN_SYNC` drift, zero active alarms, empty queues, and migrations through `0020`; its only CloudFormation resource change was the existing Session Lambda code.
