...
- Date: 2026-08-19
- Latest merged product baseline: `3993019`; PR #271 corrects staff redeem when ROLLER booking detail omits its venue and retains the PIN session for non-authentication redeem failures.
- Latest deployed baseline: release run `32226856610`, Park run `32227246989`, and public run `32227590881` promoted `3993019`; exact release, AWS/Cloudflare, drift, alarms, queues, migrations, phone/staff, Cognito, and Apple association checks passed.
...
- Product/runtime state: park-test has 202 resources, migrations through `0020`, and 27 API routes. Both public origins run `3993019`; guest loaded live Roller capacity and staff loaded PIN/admin with the correct Cognito callback. A real public purchase and personal-PIN redemption completed once without the prior logout/retry loop. The success view then advanced automatically too quickly, which remains a separate presentation defect. Phone/Park expose Weekday Combo `1242135`/`1242136`, not retired ComboDeal. Visible authenticated Handoff refreshes every five seconds, preserves compact pending/selected detail, and enables `Slutför` only after authoritative tickets. T-30 is disarmed and the general send gate is false.
...
- Issue #264 makes technical `park-test` Nacka's sharp pilot backend without changing its AWS/data identity; issue #272's protected public run `32227590881` most recently promoted both public origins. Multi-park remains separate.
...
- The stack has 202 resources and 27 routes (6 IAM, 4 JWT, 17 Lambda-protected). Selected and deployed templates match, CloudFormation drift is `IN_SYNC`, all alarms are `OK`, and the Roller operations and webhook queues/DLQs are empty.
- Routine Park/public deployment is GitHub-native. Release `32226856610`, Park run `32227246989`, and public run `32227590881` promoted `3993019` with exact verification; direct Cloudflare Git is off on all four retained projects. The Redeem handler falls back to authenticated `/venues/me` venue context when booking detail omits `venueId`, and staff logout is limited to explicit authentication/session failures.
