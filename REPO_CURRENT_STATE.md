...
- Product/runtime state: park-test has 202 resources, migrations through `0020`, and 27 API routes. Both public origins run `96022c8`; guest loaded live Roller capacity and staff loaded PIN/admin with the correct Cognito callback. A real public purchase and personal-PIN redemption completed without the prior logout/retry loop. The mobile success view now remains mounted after queue removal until staff explicitly returns or scans again; its physical retest is pending. Phone/Park expose Weekday Combo `1242135`/`1242136`, not retired ComboDeal. Visible authenticated Handoff refreshes every five seconds, preserves compact pending/selected detail, and enables `Slutför` only after authoritative tickets. T-30 is disarmed and the general send gate is false.
