...
- Date: 2026-08-07
- Latest merged product baseline: `b272c5f` after PR #228 separated the kiosk Booking Costs payload from the terminal-bearing Create Draft payload; the shared ROLLER card-present contract from PR #225 and controlled guest-domain changes from PRs #221-#222 remain intact.
- Latest deployed product baseline: protected run `31200740299` promoted immutable release `b272c5f` to the normal park-test stack, phone, and admin targets. The separate controlled `https://checkin.jumpyard.se` alias still serves selected phone artifact `9ffe379` against the same API. Issue #216 controlled automatic T-30 email remains deployed and disarmed.
...
- Product/runtime state: park-test has 202 resources and migrations through `0018`. API Gateway has 27 routes, including the deployed kiosk draft-finalize contract, and retains all four approved origins while blocking unapproved origins. The server-owned kiosk terminal alias is configured without exposing its value; Booking Costs now omits the terminal-only field while Create Draft retains it. A supervised physical payment proof remains pending. The controlled guest alias remains on exact release `9ffe379`; Love's manual iPhone payment result remains pending. The T-30 hash-only control is disarmed and the general send gate is false.
...
- The stack has 202 resources and 27 routes (6 IAM, 4 JWT, 17 Lambda-protected). Six restricted handler database identities plus one lifecycle identity remain deployed. The intake/worker are `Active`, worker concurrency is one, recovery is enabled every five minutes but selects only rows below five attempts, queue/DLQ are empty, the selected and deployed templates match, and drift is `IN_SYNC`.
- Routine park-test deployment is GitHub-native. Issue #216 release `30811646770` and protected promotion `30812035906` kept 199 resources, changed only `LookupHandler`, applied no migration, and passed all post-deploy checks without rollback.
...
- T0195 migrations `0010`-`0012`, T0196 migrations `0013`-`0014`, T0197 migrations `0015`-`0016`, issue #212 migration `0017`, and issue #224 migration `0018` are deployed to park-test. `0018` adds only safe kiosk payment-channel/attempt state and a partial unique attempt index to existing prepayment drafts. The earlier lifecycle dry-run predates the booking-index import; lifecycle apply must be replanned/recounted and remains separately gated.
