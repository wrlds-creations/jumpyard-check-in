...
- Date: 2026-08-17
- Latest merged product baseline: `506cbcb`; PR #236 changed the server-owned ROLLER kiosk terminal mapping from a legacy string to a fail-closed `deviceId`/`terminalId` object.
- Latest deployed product baseline: protected run `31876392673` promoted immutable release `506cbcb`; only `BookingHandler` and CDK metadata changed and migrations remained off. AWS/CDK and Cloudflare deployment completed; final verification reported only the known cost-center tag drift owned by #233.
...
- Product/runtime state: park-test has 202 resources and migrations through `0018`. API Gateway has 27 routes, including the kiosk draft-finalize contract. Missing booking venue is accepted only after authenticated Nacka `50871` provider verification; a physical kiosk lookup reached the ready screen. The park-test secret now holds the complete server-owned terminal object with tips disabled and no static amount; physical payment proof remains pending. The controlled check-in alias passed Love's 2026-08-17 iPhone payment/Apple Pay test but remains park-test-backed. The T-30 control is disarmed and the general send gate is false.
