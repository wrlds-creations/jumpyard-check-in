...
- Date: 2026-08-17
- Latest merged product baseline: `662564b`; PR #242 records definitive kiosk approval first and reconciles ROLLER booking confirmation asynchronously without changing phone ecommerce.
...
- Product/runtime state: park-test has 202 resources and migrations through `0018`. API Gateway has 27 routes, including the kiosk draft-finalize contract. Missing booking venue is accepted only after authenticated Nacka `50871` provider verification; physical lookup and P400 approval are proven. Main now contains bounded background reconciliation and migration `0019`, but deployment/retest remain pending. The controlled check-in alias passed Love's 2026-08-17 iPhone payment/Apple Pay test but remains park-test-backed. The T-30 control is disarmed and the general send gate is false.
