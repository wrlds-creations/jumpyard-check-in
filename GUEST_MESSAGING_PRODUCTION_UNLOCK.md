...
| SES production access | `ProductionAccessEnabled=true` | Production access is approved in `eu-north-1`; application delivery remains separately gated. |
| SES sending | `SendingEnabled=true` | The account can send; project configuration-set and application gates still control use. |
| SES quota | `50,000/day`, `14/sec` | Provider quota exceeds the approved extreme pilot model. |
| SES identities | `love@wrlds.com` verified; `jumpyard.se` verified with DKIM `SUCCESS` | The Nacka sender domain is ready for controlled transactional delivery. |
...
- Park-test application/manual sends remain disabled. The 2026-07-22 two-address proof and the explicitly approved 2026-08-02 Gmail-only final-design proof completed and restored configuration-set sending to false.
...
3. João applied the exact three generated DKIM CNAME records; public DNS and SES report verified/SUCCESS.
4. AWS approved SES production access at 50,000/day and 14/second.
5. The separately approved controlled proofs reached three cumulative sends and three deliveries with zero provider failure events, then restored configuration-set sending to false.
6. Love approved the final single-CTA rendering from the delivered 2026-08-02 Gmail message. Production delivery must replace the park-test link with `https://checkin.jumpyard.se/` only in separately approved production/cutover work, after the domain's payment and Apple Pay registration/approval is reverified.
7. Keep automatic/manual application sends off at T0200 closeout and let T0201 separately review unattended timing, application audit/deduplication, and kill-switch behavior.
